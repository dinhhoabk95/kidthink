# Checklist — Task #132: Engine `GT-003` Kéo vào đích

> Kế hoạch: [`132-engine-gt-003-plan.md`](132-engine-gt-003-plan.md).
> Vị trí **4 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-003`

- [x] `level_count` — kỳ vọng **27**. Đo: **27**.
- [x] `min_band_count` trên 3 band hợp lệ: **3-4:11 4-5:11 5-6:5 min=5**.
- [x] `thinking_span`: **6**.
- [x] `what_span`: **8**.
- [x] `theme_span`: **7**.
- [x] `difficulty_span`: **4**.
- [x] `out_of_band_count`: **0**.
- [x] Số level `GT-003` không parse được `content_contract`: **27**.
- [x] Số level `GT-003` có `access_tier` là `free` hoặc `login`: **8**.
- [x] Đọc `packages/game-engine/src/templates/GT-003/template.ts` và `session.ts` (77 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-003.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT003`, `engine: drag-to-container`, `batch: mvp`.
...
- [x] Viết mới mục 6 — `BR-E003-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E003-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-003` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-003` trong bậc thang.

## WP132.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-003/`
...
- [x] Thêm `GT-003` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-003` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP132.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-003`

- [x] Đo: bao nhiêu trong 27 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-003` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP132.4 — Level ngoài band
...
- [ ] Mở một màn `GT-003` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-003` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-003` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-003` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-003` parse được contract.
- [x] 4. `out_of_band_count` của `GT-003` = 0.
- [x] 5. `check:engine-depth` bậc 1 xanh cho `GT-003`.
- [x] 6. `GT-003` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-003` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=27 band=3-4:11 4-5:11 5-6:5 min=5 thinking=6 what=8 theme=7 diff=4
- `BR-E003-*` đã viết: ................
- Số level trượt parse, trước và sau: **27**
- Số level đã soạn thêm: ................
- Chi phí thật của WP132.2 (giờ): ................
