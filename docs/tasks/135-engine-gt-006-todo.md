...
- [ ] `min_band_count` trên 1 band hợp lệ (`5-6`): ................
- [ ] `thinking_span`: ................
- [ ] `what_span`: ................
- [ ] `theme_span`: ................
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-006` không parse được `content_contract`: ................
- [ ] Số level `GT-006` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-006/template.ts` và `session.ts` (78 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT006`, `engine: sequence-order`, `batch: mvp`.
...
- [ ] Viết mới mục 6 — `BR-E006-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E006-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-006` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-006` trong bậc thang.
...
- [ ] Phép kiểm 2 — vùng chạm ≥ sàn chạm, đo ở 1 band hợp lệ (`5-6`) (`BR-ERC-04`).
...
- [ ] Thêm `GT-006` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-006` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [ ] 1. Spec `GT-006` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-006` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
