# Checklist — Task #152: Engine `GT-023` Lắp ghép hình thể

> Kế hoạch: [`152-engine-gt-023-plan.md`](152-engine-gt-023-plan.md).
> Vị trí **23 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-023`
...
- [x] Số level `GT-023` không parse được `content_contract`: **0**.
- [x] Số level `GT-023` có `access_tier` là `free` hoặc `login`: **2**.
- [x] Đọc `packages/game-engine/src/templates/GT-023/template.ts` và `session.ts` (119 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-023.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT023`, `engine: construct`, `batch: legacy-v1`.
...
- [x] Viết mới mục 6 — `BR-E023-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E023-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-023` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-023` trong bậc thang.

## WP152.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-023/`
...
- [x] Thêm `GT-023` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-023` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP152.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-023`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-023` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP152.4 — Level ngoài band
...
- [ ] Mở một màn `GT-023` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-023` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-023` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-023` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-023` parse được contract.
- [x] 4. `out_of_band_count` của `GT-023` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-023`.
- [x] 6. `GT-023` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-023` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=3-4:1 4-5:1 5-6:1 min=1 thinking=1 what=1 theme=3 diff=3
- `BR-E023-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP152.2 (giờ): ................
