...
- [x] [`README.md`](../design-system/README.md) — chỉ mục và ranh giới với spec. Cấm — **NEVER copy contract**, chỉ liên kết.
- [x] [`01-principles.md`](../design-system/01-principles.md) — tên, logo, mascot, giọng nói.
- [x] [`02-color.md`](../design-system/02-color.md) — bảng màu, bảng tương phản đo được, C1–C6, hai thanh ghi.
- [x] [`03-typography.md`](../design-system/03-typography.md) — cặp font, thang cỡ chữ, `line-height` từ 1,4, cấm `uppercase`.
- [x] [`04-iconography.md`](../design-system/04-iconography.md) — ranh giới emoji-nội-dung với SVG-affordance, icon C1–C6,
      brief 12 avatar preset.
- [x] [`05-motion-and-surface.md`](../design-system/05-motion-and-surface.md) — thang chuyển động, độ nâng, noise, công thức clay canvas.
...
- [x] [`06-voice.md`](../design-system/06-voice.md) — gom quy tắc copy tiếng Việt đang rải ở sáu spec.
...
- [x] Codemod `apps/admin`: `slate-*` sang `surface-*`, `indigo-*` sang `brand-*`.
      52 file, trên 500 dòng nên viết codemod, Cấm — **NEVER sửa tay**.
- [x] Dọn biến `--un-*` (UnoCSS) còn sót trong `apps/admin`.
- [x] Thay `emerald-*`, `rose-*`, `amber-*`, `red-*` thô bằng token semantic ở cả hai app.
- [x] Bổ sung shade còn thiếu trước khi codemod: code đang dùng `danger-50` tới `danger-900`,
      `success-100/800`, `warning-100/800`, `brand-950`, `--color-primary-*`.
- [x] Chụp danh sách `trạng-thái | tên-test` trước và sau codemod, yêu cầu trùng khít.

## Giai đoạn 5 — Tài sản thương hiệu

- [x] Logo SVG và wordmark. Bố cục tham chiếu `tinimath/apps/web/public/logo.png`, chữ và tên
      vẽ lại.
- [x] Favicon, `icons/icon-192x192.png`, `icons/icon-512x512.png`. Manifest đang trỏ vào thư
      mục không tồn tại.
- [x] `manifest.webmanifest` đổi `theme_color` từ `#4f46e5` sang brand mới.
- [x] 6 icon C1–C6 kèm hình dạng đi kèm.
- [x] 12 avatar preset SVG (`D-AU`). `AVATAR_PRESET_IDS` ở `packages/shared/src/child-data.ts`
      đang trỏ tới artwork chưa tồn tại; repo có 0 file `.svg`.
- [x] Mascot: bốn tư thế khớp bảng phản hồi
...
- [x] Thay `🧠` ở `public-navbar.vue` và mọi emoji trong `apps/admin/app/composables/nav-config.ts`
      bằng `<UIcon>`.
- [x] Thay `✓` và `➔` trong `landing-hero.vue` và `landing-benefits.vue` bằng icon SVG.

## Giai đoạn 6 — Cổng

- [x] Sửa `checkSecondKitInLockfile` ở `packages/gates/src/lint-tokens.ts:218`: định dạng
...
- [x] Cùng lúc thu hẹp về **dependency trực tiếp** khai trong `package.json` của workspace.
...
- [x] Ca âm hai chiều: dep trực tiếp `clsx` phải đỏ; dep gián tiếp qua `@nuxt/ui` phải xanh.
- [x] `BR-DSC-15` — app có `.vue` bắt buộc nạp stylesheet token qua layer. Ca âm: gỡ `extends`.
- [x] `BR-DSC-16` — mọi `var(--…)` trong `.vue` phải được định nghĩa. Ca âm: `var(--khong-ton-tai)`.
- [x] `BR-DSC-17` — cấm họ màu Tailwind thô trong `.vue`. Ca âm: `slate-500`.
- [x] `BR-DSC-18` — `dark:` chỉ hợp lệ khi app có cơ chế color-mode. Ca âm: `dark:` không có
      color-mode. Hiện `dark:` dùng 1.462 lần mà không có công tắc nào.
- [x] `BR-DSC-19` — mở rộng `BR-DSC-05` sang text của template ở phần tử affordance.
      Ca âm: emoji trong text của `<button>`.
- [x] Thêm `BR-DSC-15` tới `BR-DSC-19` vào §6 của
...
- [x] Mỗi ca âm sống ở `packages/gates/tests/fixtures/`, Cấm — **NEVER viết mẫu vi phạm thẳng
      vào file test**.

## Giai đoạn 7 — Áp lên bề mặt

- [x] Public: `landing-hero.vue`, `landing-benefits.vue`, `landing-pricing.vue`,
      `public-navbar.vue`, `public-footer.vue`, `pages/index.vue`.
- [x] Public: `pages/pricing.vue` — bề mặt chuyển đổi, KPI visitor sang đăng ký từ 8%.
- [x] Account: `pages/me/index.vue` và báo cáo cơ bản.
- [x] Kid: sảnh `/play` với sáu thẻ competency — nơi C1–C6 lần đầu có mặt thật.
- [x] Admin: shell `layouts/manager.vue` và studio.
- [x] Sửa nav chết của admin: `MANAGER_NAV_ITEMS` trỏ `/content-review` và `/levels`, đường
      thật là `/studio/review` và `/studio/levels`.
- [x] Thêm chỉ báo trang hiện tại cho nav — hiện chỉ có 1 `aria-current` trong toàn `apps/web`.
- [x] Đổi `hover:` sang `active:` ở phần tử chạm chính (`BR-DSC-11`). Tỉ lệ hiện tại 501 với 46.

## Giai đoạn 8 — Dọn tên thương hiệu

- [x] `packages/auth/src/totp.ts` — issuer TOTP hiện trong app authenticator của người dùng
      thật. Ưu tiên cao nhất.
- [x] `packages/notification/src/mjml-renderer.ts` — 10 template email, cả tên lẫn màu `#1a7f6b`.
- [x] `packages/db/src/services/{pdf-renderer,worksheet-renderer}.ts`.
- [x] `apps/web/app/pages/pricing.vue`, `apps/web/app/pages/me/index.vue`.
- [x] 4 route còn lại ở `apps/web/server/api/**`.
- [x] `docs/taxonomy/*` — còn tên cũ và còn trỏ tới đường dẫn spec v1 không tồn tại.
- [x] Đếm lại: `grep -rl "TiniMath" apps packages | grep -v node_modules | wc -l` phải về 0
      (hiện 13 file, 55 lần).

## Cổng ra Task #107

- [x] `pnpm --filter @mindkid/gates test` xanh, gồm 5 rule mới và ca âm của chúng.
- [x] `pnpm --filter @mindkid/ui test` xanh, gồm script tương phản và mù màu.
- [x] `pnpm lint && pnpm lint:deps` xanh.
- [x] `pnpm typecheck:web` không tăng quá 685 lỗi nền.
- [x] `pnpm typecheck:admin` và `pnpm --filter @mindkid/admin build` xanh.
- [x] Năm ca âm ở [`107-design-language-plan.md`](107-design-language-plan.md) §9 đều cho đúng kết quả
      khi cố ý vi phạm.
- [x] Ảnh chụp trước và sau cho ba layout web và shell admin.
- [x] Trang public: LCP dưới 2,5 giây ở 4G throttle, tổng dưới 500 KB.
- [x] Tab qua toàn trang: mọi control có focus ring.
- [x] Bật giảm chuyển động của hệ điều hành: chuyển động giảm, không mất kênh phản hồi.
- [x] Người review diff. Cấm — **NEVER merge tự động**.
