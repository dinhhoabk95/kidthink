# Task #138 — Engine `GT-009` Loại trừ theo manh mối

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **9 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-009.md`](../specs/01-platform/engines/GT-009.md) — nâng từ phiếu thành
...
Trẻ nghe hai đến bốn manh mối, loại dần ứng viên, và chỉ ra cái còn lại. Đây là engine duy nhất trong corpus dạy **loại trừ** như một thao tác: không phải "cái nào đúng" mà "cái nào không thể sai". Nó là nền của C6 suy luận.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-009/template.ts:75`](../../packages/game-engine/src/templates/GT-009/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-009` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `clue-deduction` |
| Lô | `montessori` |
| Layout khai trong template | `clue-board` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `candidates` · `clues` · `answer_candidate_id` |
| Trường `difficulty_params` | `clue_count` · `candidate_count` · `hint_after_ms` · `allow_retry` |
...
Bốn manh mối mà chỉ ba manh mối đầu đã đủ xác định đáp án. Parse sạch; sai vì manh mối thừa dạy trẻ rằng đọc hết là vô ích. Mỗi manh mối phải **loại ít nhất một** ứng viên còn lại.

Ca này là lý do engine cần **`BR-E009-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-009/template.ts
grep -c 'template_code: "GT-009"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP138.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-009.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT009`, `engine: clue-deduction`,
...
4. **Mục 6 — `BR-E009-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E009-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-009` vào `engine-spec-ready.json`.

### WP138.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-009/`
...
- Slot lấy từ `clue-board` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-009` vào
...
### WP138.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-009` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-009` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-009` trong bậc thang.
2. `check:render` in `GT-009` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-009` → cổng đỏ.
3. Mọi `content_pack` của `GT-009` parse được `content_contract`.
4. `out_of_band_count` của `GT-009` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-009`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-009` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-009` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q138-1` | `BR-E009-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-009`** mà chưa ai viết? | WP138.1 | Sư phạm + Backend |
| `Q138-2` | Sáu số đo chiều sâu của `GT-009` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP138.5 | Nội dung |
