# Checklist — Task #150: Engine `GT-021` Hoàn thiện đối xứng

> Kế hoạch: [`150-engine-gt-021-plan.md`](150-engine-gt-021-plan.md).
> Vị trí **21 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-021`
...
- [x] Số level `GT-021` không parse được `content_contract`: **0**.
- [x] Số level `GT-021` có `access_tier` là `free` hoặc `login`: **1**.
- [x] Đọc `packages/game-engine/src/templates/GT-021/template.ts` và `session.ts` (109 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-021.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT021`, `engine: mirror-complete`, `batch: legacy-v1`.
...
- [x] Viết mới mục 6 — `BR-E021-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E021-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-021` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-021` trong bậc thang.

## WP150.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-021/`
...
- [x] Thêm `GT-021` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-021` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP150.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-021`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-021` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP150.4 — Level ngoài band
...
- [ ] Mở một màn `GT-021` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-021` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-021` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-021` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-021` parse được contract.
- [x] 4. `out_of_band_count` của `GT-021` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-021`.
- [x] 6. `GT-021` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-021` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
