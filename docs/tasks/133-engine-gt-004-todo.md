# Checklist — Task #133: Engine `GT-004` Phân loại vào nhóm

> Kế hoạch: [`133-engine-gt-004-plan.md`](133-engine-gt-004-plan.md).
> Vị trí **5 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-004`

- [ ] `level_count` — kỳ vọng **21**.
...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-004` không parse được `content_contract`: ................
- [ ] Số level `GT-004` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-004/template.ts` và `session.ts` (78 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-004.md`

- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT004`, `engine: sort-groups`, `batch: mvp`.
...
- [ ] Viết mới mục 6 — `BR-E004-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E004-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-004` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-004` trong bậc thang.

## WP133.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-004/`
...
- [ ] Thêm `GT-004` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-004` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP133.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-004`

- [ ] Đo: bao nhiêu trong 21 level trượt, thiếu trường nào.
- [ ] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-004` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP133.4 — Level ngoài band

**Cỡ:** S

- [ ] Đo level `GT-004` gắn band bị cấm (`3-4`).
...
- [ ] `out_of_band_count` của `GT-004` về **0**.
...
- [ ] Mở một màn `GT-004` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-004` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [ ] 1. Spec `GT-004` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-004` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-004` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-004` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-004`.
...
- [ ] Cập nhật hàng `GT-004` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
