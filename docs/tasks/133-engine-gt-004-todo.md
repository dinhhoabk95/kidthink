...
- [x] `theme_span`: **5**.
- [x] `difficulty_span`: **3**.
- [x] `out_of_band_count`: **3**.
- [x] Số level `GT-004` không parse được `content_contract`: **21**.
- [x] Số level `GT-004` có `access_tier` là `free` hoặc `login`: **4**.
- [x] Đọc `packages/game-engine/src/templates/GT-004/template.ts` và `session.ts` (78 dòng).
...
- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT004`, `engine: sort-groups`, `batch: mvp`.
...
- [x] Viết mới mục 6 — `BR-E004-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E004-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-004` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-004` trong bậc thang.
...
- [x] Thêm `GT-004` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-004` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [x] 1. Spec `GT-004` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-004` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
...
- [x] 6. `GT-004` có ≥1 level `free` hoặc `login`.
...
- Sáu số đo trước và sau: level=21 band=4-5:10 5-6:8 min=8 thinking=4 what=6 theme=5 diff=3
- `BR-E004-*` đã viết: ................
