...
> ⚠️ **Checkbox trong file này không được tick lại sau khi làm.** Nguồn sự thật là git:
...
> 1. `data-model-overview` §Ràng buộc chờ — **chưa tạo**, 11 quyết định T8–T12 không ở đâu tập trung.
> 2. `SPEC.md` §13 Cổng ra P0 — **chưa có** dòng neo D-W, cũng chưa có dòng backup/monitoring (Q4).
