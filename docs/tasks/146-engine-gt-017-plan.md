# Task #146 — Engine `GT-017` Xếp khối và phối cảnh

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **17 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-017.md`](../specs/01-platform/engines/GT-017.md) — nâng từ phiếu thành
...
Trẻ nhìn một khối lập phương xếp chồng và trả lời về nó: có bao nhiêu khối, nhìn từ trên xuống thấy gì, khối nào bị che. Engine này dạy **suy ra cái không nhìn thấy từ cái nhìn thấy** — bước đầu của tư duy không gian ba chiều.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-017/template.ts:62`](../../packages/game-engine/src/templates/GT-017/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-017` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `block-stack` |
| Lô | `montessori` |
| Layout khai trong template | `split-columns` · `grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `model` · `question` · `options` |
| Trường `difficulty_params` | `hidden_cube_count` · `distractor_count` · `allow_rotate` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `isometric-system` |
| `session.ts` | 142 dòng |
...
`hidden_cube_count` = 3 với `allow_rotate` = false ở level đầu tiên của một skill. Parse sạch; sai vì trẻ chưa có mô hình khối chồng và không có cách kiểm chứng, nên câu trả lời là đoán.

Ca này là lý do engine cần **`BR-E017-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-017/template.ts
grep -c 'template_code: "GT-017"' -r packages/db/src/seed-content
...
**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-017.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT017`, `engine: block-stack`,
...
4. **Mục 6 — `BR-E017-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E017-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-017` vào `engine-spec-ready.json`.

### WP146.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-017/`
...
- Slot lấy từ `split-columns` · `grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-017` vào
...
1. Mở một màn `GT-017` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-017` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-017` trong bậc thang.
2. `check:render` in `GT-017` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-017` → cổng đỏ.
3. Mọi `content_pack` của `GT-017` parse được `content_contract`.
4. `out_of_band_count` của `GT-017` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-017`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-017` có ≥1 level `free` hoặc `login`.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q146-1` | `BR-E017-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-017`** mà chưa ai viết? | WP146.1 | Sư phạm + Backend |
| `Q146-2` | Sáu số đo chiều sâu của `GT-017` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP146.5 | Nội dung |
