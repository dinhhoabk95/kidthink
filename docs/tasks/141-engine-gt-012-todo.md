...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-012` không parse được `content_contract`: ................
- [ ] Số level `GT-012` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-012/template.ts` và `session.ts` (148 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT012`, `engine: flash-recall`, `batch: montessori`.
...
- [ ] Viết mới mục 6 — `BR-E012-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E012-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-012` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-012` trong bậc thang.
...
- [ ] Thêm `GT-012` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-012` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [ ] 1. Spec `GT-012` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-012` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-012` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-012` = 0.
