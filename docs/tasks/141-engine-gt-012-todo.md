# Checklist — Task #141: Engine `GT-012` Nhìn chớp rồi nhớ lại

> Kế hoạch: [`141-engine-gt-012-plan.md`](141-engine-gt-012-plan.md).
> Vị trí **12 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-012`
...
- [x] Số level `GT-012` không parse được `content_contract`: **1**.
- [x] Số level `GT-012` có `access_tier` là `free` hoặc `login`: **3**.
- [x] Đọc `packages/game-engine/src/templates/GT-012/template.ts` và `session.ts` (148 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-012.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT012`, `engine: flash-recall`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E012-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E012-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-012` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-012` trong bậc thang.

## WP141.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-012/`
...
- [x] Thêm `GT-012` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-012` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP141.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-012`

- [x] Đo: bao nhiêu trong 4 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-012` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP141.4 — Level ngoài band
...
- [ ] Mở một màn `GT-012` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-012` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-012` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-012` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-012` parse được contract.
- [x] 4. `out_of_band_count` của `GT-012` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-012`.
- [x] 6. `GT-012` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-012` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=4 band=3-4:2 4-5:1 5-6:1 min=1 thinking=2 what=2 theme=3 diff=3
- `BR-E012-*` đã viết: ................
- Số level trượt parse, trước và sau: **1**
- Số level đã soạn thêm: ................
- Chi phí thật của WP141.2 (giờ): ................
