# Task #136 — Engine `GT-007` Tách gộp số

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **7 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-007.md`](../specs/01-platform/engines/GT-007.md) — nâng từ phiếu thành
...
Trẻ nhìn một tổng và tìm các phần cộng lại thành nó: 5 là 2 với 3, hoặc 1 với 4. Đây là mô hình trực quan của phép cộng và phép trừ **trước khi** trẻ gặp ký hiệu. Nguồn Montessori của nó là bộ thanh số và bộ hạt màu.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-007/template.ts:48`](../../packages/game-engine/src/templates/GT-007/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-007` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `number-bond` |
| Lô | `montessori` |
| Layout khai trong template | `number-bond-tree` · `ten-frame-split` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `whole` · `parts` · `options` |
| Trường `difficulty_params` | `part_count` · `distractor_count` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Không |
...
`whole` = 10 với `part_count` = 2 ở band 3-4. Parse sạch, `limits` sạch. Sai vì phạm vi số ở band 3-4 là 1–5; trẻ chưa có biểu diễn lượng cho 10 nên bài trở thành đoán.

Ca này là lý do engine cần **`BR-E007-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-007/template.ts
grep -c 'template_code: "GT-007"' -r packages/db/src/seed-content
...
**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-007.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT007`, `engine: number-bond`,
...
4. **Mục 6 — `BR-E007-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E007-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-007` vào `engine-spec-ready.json`.

### WP136.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-007/`
...
- Slot lấy từ `number-bond-tree` · `ten-frame-split` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-007` vào
...
1. Mở một màn `GT-007` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-007` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-007` trong bậc thang.
2. `check:render` in `GT-007` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-007` → cổng đỏ.
3. Mọi `content_pack` của `GT-007` parse được `content_contract`.
4. `out_of_band_count` của `GT-007` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-007`: `level_count` ≥6 và bốn `*_span` ≥2.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q136-1` | `BR-E007-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-007`** mà chưa ai viết? | WP136.1 | Sư phạm + Backend |
| `Q136-2` | Sáu số đo chiều sâu của `GT-007` hôm nay là bao nhiêu? Chỉ `level_count` = 6 là chắc; năm số còn lại phải đo ở Preflight | WP136.5 | Nội dung |
