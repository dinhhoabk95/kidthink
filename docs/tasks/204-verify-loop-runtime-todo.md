...
> ## ⚠ TRẠNG THÁI THẬT (2026-09-02, 14:05) — ĐỌC TRƯỚC
>
> Bốn slice dưới đây **đã implement rồi ĐÃ BỊ REVERT** theo yêu cầu người dùng
> (`2912806 revert: back out task 204 verify-loop work`). Chúng đang đánh dấu
> `[x] XONG` vì đó là ghi chép lúc làm — **nhưng mã KHÔNG còn trong cây**:
>
> | slice | commit gốc | trạng thái mã |
> |---|---|---|
> | `#204.1` cờ `database` | `b9a5df2` | đã revert |
> | `#204.6` `incremental` cho lưới root | `55f3436` | đã revert |
> | `#204.7` cổng pool 4 + incremental | `55f3436` | đã revert |
> | `#204.8` một đường typecheck | `0e116b2` | đã revert |
>
> Muốn lấy lại: `git revert 2912806`, hoặc `git checkout <commit> -- <file>` cho
> từng phần. Số đo và ca âm ghi trong các mục đó **vẫn đúng** — chúng đã chạy thật.
>
> Hai file kế hoạch này được khôi phục riêng (`git checkout 6e72994 -- docs/tasks/204-*`)
> vì có phiên khác đang implement `#204.5` và `#204.10` dựa vào chúng.
