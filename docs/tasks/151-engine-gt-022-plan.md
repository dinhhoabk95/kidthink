# Task #151 — Engine `GT-022` Tìm vật thể ẩn

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **22 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-022.md`](../specs/01-platform/engines/GT-022.md) — nâng từ phiếu thành
...
Trẻ tìm những vật khớp mô tả trong một khung cảnh đông vật. Engine này luyện **quét thị giác có chủ đích** và khả năng bỏ qua thứ gây nhiễu — hai thành phần của chú ý chọn lọc, thuộc C6.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-022/template.ts:32`](../../packages/game-engine/src/templates/GT-022/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-022` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `hidden-object` |
| Lô | `legacy-v1` |
| Layout khai trong template | `free-scene` · `grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `target_description` · `scene_objects` |
| Trường `difficulty_params` | `hint_after_ms` · `allow_retry` · `show_target_counter` |
| Dùng `.refine()` | Không |
| System và mechanic | `scene-system` · `selection-mechanic` |
| `session.ts` | 131 dòng |
...
`scene_objects` xếp chồng nhau đến mức một vật đích bị che quá nửa. Parse sạch; sai vì bài trở thành bài may rủi chạm. Vật đích phải hiện đủ để nhận ra, dù không dễ thấy.

Ca này là lý do engine cần **`BR-E022-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-022/template.ts
grep -c 'template_code: "GT-022"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP151.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-022.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT022`, `engine: hidden-object`,
...
4. **Mục 6 — `BR-E022-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E022-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-022` vào `engine-spec-ready.json`.

### WP151.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-022/`
...
- Slot lấy từ `free-scene` · `grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-022` vào
...
### WP151.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-022` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-022` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-022` trong bậc thang.
2. `check:render` in `GT-022` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-022` → cổng đỏ.
3. Mọi `content_pack` của `GT-022` parse được `content_contract`.
4. `out_of_band_count` của `GT-022` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-022`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-022` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-022` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q151-1` | `BR-E022-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-022`** mà chưa ai viết? | WP151.1 | Sư phạm + Backend |
| `Q151-2` | Sáu số đo chiều sâu của `GT-022` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP151.5 | Nội dung |
