# Task #132 — Engine `GT-003` Kéo vào đích

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **4 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-003.md`](../specs/01-platform/engines/GT-003.md) — nâng từ phiếu thành
...
Trẻ kéo vật vào **một** đích chứa. Cơ chế mang theo một ý nghĩa mà chạm không có: hành động "bỏ vào" là mô hình vận động của phép gộp, phép đếm có tích luỹ, và của việc thuộc về một tập hợp. Đó là lý do engine này gánh phần lớn nội dung đếm của C1.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-003/template.ts:35`](../../packages/game-engine/src/templates/GT-003/template.ts) — `BR-ESS-02` buộc spec ghi kèm
nguồn dòng này và cổng so lại với registry lúc chạy.

Lô `mvp` (MVP). Hôm nay engine có **27 level** trong corpus seed và
**chưa cài `render()`** — trẻ mở một màn `GT-003` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `drag-to-container` |
| Lô | `mvp` |
| Layout khai trong template | `top-source-bottom-target` · `left-source-right-target` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `container` · `items` |
...
Một đích chứa vẽ bằng emoji nhỏ hơn vật được kéo. Parse sạch; sai vì trẻ không nhận ra cái nhỏ chứa được cái to, và thất bại là thất bại về hình ảnh chứ không phải về nội dung.

Ca này là lý do engine cần **`BR-E003-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-003/template.ts
grep -c 'template_code: "GT-003"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP132.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-003.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT003`, `engine: drag-to-container`,
...
4. **Mục 6 — `BR-E003-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E003-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-003` vào `engine-spec-ready.json`.

### WP132.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-003/`
...
- Slot lấy từ `top-source-bottom-target` · `left-source-right-target` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-003` vào
...
### WP132.5 — Nội dung tới sàn bậc 1
...
### WP132.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-003` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-003` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-003` trong bậc thang.
2. `check:render` in `GT-003` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-003` → cổng đỏ.
3. Mọi `content_pack` của `GT-003` parse được `content_contract`.
4. `out_of_band_count` của `GT-003` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-003`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-003` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-003` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q132-1` | `BR-E003-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-003`** mà chưa ai viết? | WP132.1 | Sư phạm + Backend |
| `Q132-2` | Sáu số đo chiều sâu của `GT-003` hôm nay là bao nhiêu? Chỉ `level_count` = 27 là chắc; năm số còn lại phải đo ở Preflight | WP132.5 | Nội dung |
