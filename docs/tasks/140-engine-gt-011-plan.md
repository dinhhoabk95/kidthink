# Task #140 — Engine `GT-011` Ma trận chọn hình

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **11 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-011.md`](../specs/01-platform/engines/GT-011.md) — nâng từ phiếu thành
...
Trẻ nhìn một lưới có một ô trống, tìm quy luật theo hàng và theo cột, rồi chọn hình điền vào. Đây là dạng bài suy luận quy nạp cổ điển, và là engine duy nhất trong corpus đòi trẻ đọc **hai chiều quy luật cùng lúc**.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-011/template.ts:82`](../../packages/game-engine/src/templates/GT-011/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-011` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `matrix-choice` |
| Lô | `montessori` |
| Layout khai trong template | `matrix-3x3` · `matrix-slot-grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `matrix` · `options` |
| Trường `difficulty_params` | `grid_size` · `distractor_count` · `hint_after_ms` · `allow_retry` |
...
Ma trận có quy luật theo hàng nhưng cột thì ngẫu nhiên. Parse sạch; sai vì trẻ tìm quy luật cột, không thấy, và kết luận mình sai. Cả hai chiều phải có quy luật đọc được, hoặc `prompt` phải nói rõ chỉ đọc theo hàng.

Ca này là lý do engine cần **`BR-E011-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-011/template.ts
grep -c 'template_code: "GT-011"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP140.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-011.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT011`, `engine: matrix-choice`,
...
4. **Mục 6 — `BR-E011-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E011-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-011` vào `engine-spec-ready.json`.

### WP140.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-011/`
...
- Slot lấy từ `matrix-3x3` · `matrix-slot-grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-011` vào
...
### WP140.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-011` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-011` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-011` trong bậc thang.
2. `check:render` in `GT-011` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-011` → cổng đỏ.
3. Mọi `content_pack` của `GT-011` parse được `content_contract`.
4. `out_of_band_count` của `GT-011` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-011`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-011` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-011` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q140-1` | `BR-E011-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-011`** mà chưa ai viết? | WP140.1 | Sư phạm + Backend |
| `Q140-2` | Sáu số đo chiều sâu của `GT-011` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP140.5 | Nội dung |
