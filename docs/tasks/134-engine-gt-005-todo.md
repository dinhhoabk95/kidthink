# Checklist — Task #134: Engine `GT-005` Ghép cặp

> Kế hoạch: [`134-engine-gt-005-plan.md`](134-engine-gt-005-plan.md).
> Vị trí **3 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-005`

- [x] `level_count` — kỳ vọng **24**. Đo: **24**.
- [x] `min_band_count` trên 3 band hợp lệ: **3-4:6 4-5:8 5-6:10 min=6**.
- [x] `thinking_span`: **5**.
- [x] `what_span`: **9**.
- [x] `theme_span`: **5**.
- [x] `difficulty_span`: **4**.
- [x] `out_of_band_count`: **0**.
- [x] Số level `GT-005` không parse được `content_contract`: **24**.
- [x] Số level `GT-005` có `access_tier` là `free` hoặc `login`: **5**.
- [x] Đọc `packages/game-engine/src/templates/GT-005/template.ts` và `session.ts` (81 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-005.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT005`, `engine: pair-match`, `batch: mvp`.
...
- [x] Viết mới mục 6 — `BR-E005-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E005-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-005` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-005` trong bậc thang.

## WP134.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-005/`
...
- [x] Thêm `GT-005` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-005` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP134.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-005`

- [x] Đo: bao nhiêu trong 24 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-005` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP134.4 — Level ngoài band
...
- [ ] Mở một màn `GT-005` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-005` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-005` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-005` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-005` parse được contract.
- [x] 4. `out_of_band_count` của `GT-005` = 0.
- [x] 5. `check:engine-depth` bậc 1 xanh cho `GT-005`.
- [x] 6. `GT-005` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-005` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=24 band=3-4:6 4-5:8 5-6:10 min=6 thinking=5 what=9 theme=5 diff=4
- `BR-E005-*` đã viết: ................
- Số level trượt parse, trước và sau: **24**
- Số level đã soạn thêm: ................
- Chi phí thật của WP134.2 (giờ): ................
