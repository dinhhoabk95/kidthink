...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-005` không parse được `content_contract`: ................
- [ ] Số level `GT-005` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-005/template.ts` và `session.ts` (81 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT005`, `engine: pair-match`, `batch: mvp`.
...
- [ ] Viết mới mục 6 — `BR-E005-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E005-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-005` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-005` trong bậc thang.
...
- [ ] Thêm `GT-005` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-005` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP134.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-005`

- [ ] Đo: bao nhiêu trong 24 level trượt, thiếu trường nào.
...
- [ ] 1. Spec `GT-005` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-005` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-005` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-005` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-005`.
