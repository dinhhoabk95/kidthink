# Task #145 — Engine `GT-016` Xoay kim đồng hồ

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **16 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-016.md`](../specs/01-platform/engines/GT-016.md) — nâng từ phiếu thành
...
> - quyết định `Q116-1` — nguyên thuỷ vẽ mới (mặt đồng hồ và kim xoay)

## 1. Engine này là gì

Trẻ xoay kim đồng hồ tới một giờ cho trước, hoặc đọc giờ trên mặt đồng hồ và nối với hoạt động trong ngày. Engine này dạy **thời gian như một đại lượng đọc được**, không phải như một từ nghe được.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-016/template.ts:70`](../../packages/game-engine/src/templates/GT-016/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-016` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `clock-hands` |
| Lô | `montessori` |
...
| Trường `content_pack` | `mode` · `target_time` · `initial_time` · `options` · `activity_cards` |
| Trường `difficulty_params` | `minute_step` · `distractor_count` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `rotation-system` |
| `session.ts` | 193 dòng |
...
`minute_step` = 15 với `target_time` là 3 giờ 45. Parse sạch nếu `limits` không chặn. Sai vì đọc phút lẻ chưa thuộc chương trình mầm non; bài trở thành bài đếm vạch.

Ca này là lý do engine cần **`BR-E016-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-016/template.ts
grep -c 'template_code: "GT-016"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP145.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-016.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT016`, `engine: clock-hands`,
...
4. **Mục 6 — `BR-E016-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E016-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-016` vào `engine-spec-ready.json`.

### WP145.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-016/`
...
- Vẽ bằng 5 nguyên thuỷ của `RenderSystem` và nguyên thuỷ mới đã chốt ở `Q116-1` (mặt đồng hồ và kim xoay).
- Trạng thái thị giác riêng của engine ghi ở mục 12 của spec.
- Vùng chạm đo ở **3 band hợp lệ**: `3-4`, `4-5`, `5-6`.

Sau khi bảy phép kiểm xanh: thêm `GT-016` vào
...
### WP145.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-016` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-016` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-016` trong bậc thang.
2. `check:render` in `GT-016` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-016` → cổng đỏ.
3. Mọi `content_pack` của `GT-016` parse được `content_contract`.
4. `out_of_band_count` của `GT-016` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-016`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-016` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-016` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q145-1` | `BR-E016-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-016`** mà chưa ai viết? | WP145.1 | Sư phạm + Backend |
| `Q145-2` | Sáu số đo chiều sâu của `GT-016` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP145.5 | Nội dung |
| `Q145-3` | Nguyên thuỷ **mặt đồng hồ và kim xoay** thuộc `RenderSystem` hay thuộc thư mục engine? `BR-ERC-05` cấm vẽ tay ngoài `RenderSystem`, nên để trong engine là phải nới rule — thứ **Cấm — NEVER** | WP145.2 | Backend |
