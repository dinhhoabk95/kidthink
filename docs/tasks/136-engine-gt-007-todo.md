...
- [x] Số level `GT-007` không parse được `content_contract`: **0**.
- [x] Số level `GT-007` có `access_tier` là `free` hoặc `login`: **2**.
- [x] Đọc `packages/game-engine/src/templates/GT-007/template.ts` và `session.ts` (94 dòng).
...
- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT007`, `engine: number-bond`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E007-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E007-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-007` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-007` trong bậc thang.
...
- [x] Thêm `GT-007` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-007` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [x] 1. Spec `GT-007` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-007` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-007` parse được contract.
- [x] 4. `out_of_band_count` của `GT-007` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-007`.
- [x] 6. `GT-007` có ≥1 level `free` hoặc `login`.
...
- Sáu số đo trước và sau: level=6 band=3-4:2 4-5:2 5-6:2 min=2 thinking=2 what=1 theme=1 diff=3
- `BR-E007-*` đã viết: ................
