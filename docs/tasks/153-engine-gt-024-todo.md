...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-024` không parse được `content_contract`: ................
- [ ] Số level `GT-024` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-024/template.ts` và `session.ts` (100 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT024`, `engine: trace-path`, `batch: legacy-v1`.
...
- [ ] Viết mới mục 6 — `BR-E024-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E024-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-024` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-024` trong bậc thang.
...
- [ ] Thêm `GT-024` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-024` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [ ] 1. Spec `GT-024` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-024` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-024` parse được contract.
