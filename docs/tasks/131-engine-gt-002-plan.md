# Task #131 — Engine `GT-002` Chọn nhiều đáp án

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **2 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-002.md`](../specs/01-platform/engines/GT-002.md) — nâng từ phiếu thành
...
Trẻ nghe một tiêu chí, chạm vào **mọi** vật thoả tiêu chí đó, rồi xác nhận. Khác `GT-001` ở một điểm quyết định: trẻ phải **giữ tiêu chí trong trí nhớ làm việc** suốt nhiều lượt chạm, và tự biết khi nào xong. Đó là lý do band 3-4 bị cấm.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-002/template.ts:30`](../../packages/game-engine/src/templates/GT-002/template.ts) — `BR-ESS-02` buộc spec ghi kèm
nguồn dòng này và cổng so lại với registry lúc chạy.

Lô `mvp` (MVP). Hôm nay engine có **27 level** trong corpus seed và
**chưa cài `render()`** — trẻ mở một màn `GT-002` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `tap-select-multi` |
| Lô | `mvp` |
| Layout khai trong template | `grid-2x4` · `flex-wrap` |
| Band engine **cấm** | `3-4` |
| Band hợp lệ | `4-5`, `5-6` |
| `limits` | `item_count: [3, 8], distractor_count: [1, 5], target_count: [2, 5]` |
| `requires_tap_fallback` | `false` |
| `asset_kinds` | `emoji` · `image` · `audio` |
| Trường `content_pack` | `target_criterion` · `items` |
| Trường `difficulty_params` | `distractor_count` · `target_count` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Không |
| System và mechanic | `selection-mechanic` |
| `session.ts` | 57 dòng |
| Level trong corpus | **27** |
...
`target_criterion` là "các con vật" nhưng `items` chứa cả cá và cả tàu thuỷ dưới nước. Parse sạch; sai vì tiêu chí có vùng xám, và trẻ chạm đúng theo cách hiểu của mình vẫn bị chấm sai. Tiêu chí của engine này phải **nhị phân không tranh cãi được** ở lứa 4–6.

Ca này là lý do engine cần **`BR-E002-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-002/template.ts
grep -c 'template_code: "GT-002"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP131.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-002.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT002`, `engine: tap-select-multi`,
...
4. **Mục 6 — `BR-E002-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E002-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-002` vào `engine-spec-ready.json`.

### WP131.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-002/`
...
- Slot lấy từ `grid-2x4` · `flex-wrap` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-002` vào
...
### WP131.3 — Nợ `content_pack` của engine

**Cỡ:** S · **Ranh giới PR:** `packages/db/src/seed-content`

Cổng 1 sau [`Task #117`](117-seed-gate-truth-plan.md) parse `content_pack` bằng
`content_contract` thật. Đo lại số level `GT-002` trượt, rồi sửa **của riêng engine này**.

1. Đo: bao nhiêu trong 27 level trượt, thiếu trường nào.
2. Sửa bằng **version mới**; cấm — NEVER `UPDATE` bản đã publish (`BR-CSA-01`).
3. Con số trượt của `GT-002` về 0.

Nếu đo ra 0 trượt: ghi lại con số và bỏ WP này, kèm lý do.

### WP131.4 — Level ngoài band của engine

**Cỡ:** S · **Ranh giới PR:** `packages/db/src/seed-content`

Engine này khai `banned_age_bands: [`3-4`]`. Đo level `GT-002` đang gắn band bị cấm, rồi áp luật phân loại đường A / đường B do [`Task #118`](118-band-violation-cleanup-plan.md) chốt.

### WP131.5 — Nội dung tới sàn bậc 1
...
### WP131.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-002` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-002` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-002` trong bậc thang.
2. `check:render` in `GT-002` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-002` → cổng đỏ.
3. Mọi `content_pack` của `GT-002` parse được `content_contract`.
4. `out_of_band_count` của `GT-002` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-002`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-002` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-002` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q131-1` | `BR-E002-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-002`** mà chưa ai viết? | WP131.1 | Sư phạm + Backend |
| `Q131-2` | Sáu số đo chiều sâu của `GT-002` hôm nay là bao nhiêu? Chỉ `level_count` = 27 là chắc; năm số còn lại phải đo ở Preflight | WP131.5 | Nội dung |
