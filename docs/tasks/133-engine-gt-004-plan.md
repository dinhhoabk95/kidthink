# Task #133 — Engine `GT-004` Phân loại vào nhóm

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **5 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-004.md`](../specs/01-platform/engines/GT-004.md) — nâng từ phiếu thành
...
Trẻ chia một tập vật vào 2–4 nhóm có nhãn. Khác `GT-003` ở chỗ đích không còn là một: trẻ phải giữ nhiều tiêu chí song song và **quyết định cho từng vật**. Đây là engine trung tâm của C5 phân loại và là ví dụ mà mục 1 của [`game-template-contract.md`](../specs/01-platform/game-template-contract.md) dùng để minh hoạ tách cơ chế khỏi nội dung: cùng một `GT-004`, đổi `groups` là đổi competency.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-004/template.ts:70`](../../packages/game-engine/src/templates/GT-004/template.ts) — `BR-ESS-02` buộc spec ghi kèm
nguồn dòng này và cổng so lại với registry lúc chạy.

Lô `mvp` (MVP). Hôm nay engine có **21 level** trong corpus seed và
**chưa cài `render()`** — trẻ mở một màn `GT-004` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `sort-groups` |
| Lô | `mvp` |
| Layout khai trong template | `multi-bucket-bottom` · `split-columns` |
| Band engine **cấm** | `3-4` |
| Band hợp lệ | `4-5`, `5-6` |
| `limits` | `item_count: [4, 10], distractor_count: [0, 3], target_count: [4, 10]` |
| `requires_tap_fallback` | `true` |
| `asset_kinds` | `emoji` · `image` · `audio` |
| Trường `content_pack` | `groups` · `items` |
| Trường `difficulty_params` | `distractor_count` · `hint_after_ms` · `allow_retry` · `shuffle_items` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `placement-mechanic` |
| `session.ts` | 78 dòng |
...
Ba nhóm với nhãn `to` · `nhỏ` · `màu đỏ`. Parse sạch, `refine` sạch. Sai vì hai nhóm đầu thuộc một tiêu chí và nhóm thứ ba thuộc tiêu chí khác — một vật đỏ và to thuộc hai nhóm cùng lúc. Mọi nhóm trong một level phải cắt theo **cùng một** tiêu chí.

Ca này là lý do engine cần **`BR-E004-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-004/template.ts
grep -c 'template_code: "GT-004"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP133.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-004.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT004`, `engine: sort-groups`,
...
4. **Mục 6 — `BR-E004-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E004-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-004` vào `engine-spec-ready.json`.

### WP133.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-004/`
...
- Slot lấy từ `multi-bucket-bottom` · `split-columns` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-004` vào
...
### WP133.3 — Nợ `content_pack` của engine

**Cỡ:** S · **Ranh giới PR:** `packages/db/src/seed-content`

Cổng 1 sau [`Task #117`](117-seed-gate-truth-plan.md) parse `content_pack` bằng
`content_contract` thật. Đo lại số level `GT-004` trượt, rồi sửa **của riêng engine này**.

1. Đo: bao nhiêu trong 21 level trượt, thiếu trường nào.
2. Sửa bằng **version mới**; cấm — NEVER `UPDATE` bản đã publish (`BR-CSA-01`).
3. Con số trượt của `GT-004` về 0.

Nếu đo ra 0 trượt: ghi lại con số và bỏ WP này, kèm lý do.

### WP133.4 — Level ngoài band của engine

**Cỡ:** S · **Ranh giới PR:** `packages/db/src/seed-content`

Engine này khai `banned_age_bands: [`3-4`]`. Đo level `GT-004` đang gắn band bị cấm, rồi áp luật phân loại đường A / đường B do [`Task #118`](118-band-violation-cleanup-plan.md) chốt.

### WP133.5 — Nội dung tới sàn bậc 1
...
### WP133.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-004` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-004` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-004` trong bậc thang.
2. `check:render` in `GT-004` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-004` → cổng đỏ.
3. Mọi `content_pack` của `GT-004` parse được `content_contract`.
4. `out_of_band_count` của `GT-004` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-004`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-004` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-004` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q133-1` | `BR-E004-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-004`** mà chưa ai viết? | WP133.1 | Sư phạm + Backend |
| `Q133-2` | Sáu số đo chiều sâu của `GT-004` hôm nay là bao nhiêu? Chỉ `level_count` = 21 là chắc; năm số còn lại phải đo ở Preflight | WP133.5 | Nội dung |
