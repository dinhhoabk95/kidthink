...
- [x] Cấm **KHÔNG** port `biome.json` 12.9K và `biome.base.jsonc` (D-H: preset ultracite thay)
- [x] Cấm **KHÔNG** port `src/constants.ts` (D-C: `COOKIE_PREFIXES` = auth v1, `superadmin`, prefix `tinimath_`)
...
- [x] Lưu ý: Sửa D-O: `node_modules` phải ở `doNotFollow`, **không** `exclude` — `exclude` xoá cạnh làm 2/3 rule xanh giả

## T7 — Wiring script `package.json` (sau T1, song song được song song)
- [x] `lint` `lint:fix` `lint:tokens` `typecheck` `check` `test` `format` theo [`SPEC.md`](../SPEC.md) §7
- [x] Lưu ý: `lint` = **`biome check .`**, không `ultracite check .` — wrapper exit 0 khi có lỗi (D-H b, đo trên Biome 2.5.7/2.5.5/2.4.0)
- [x] Cấm **Chưa** khai `db:*` / `dev*` / `build` (trỏ vào chỗ trống = xanh giả)
...
- [x] Cấm **Bỏ step `pnpm build`** của v1 (D-E: skeleton rỗng → xanh giả)
- [x] Mọi lệnh CI gọi đều **đã chạy xanh tại chỗ**
- [x] ~~Tạm dừng CI chạy thật trên remote~~ — không còn áp dụng. Quyết định người dùng 2026-08-06
...
- [x] `pnpm-workspace.yaml`: `allowBuilds: {lefthook: false}` — binary tới từ
  `optionalDependencies` theo platform, không cần postinstall build. Lưu ý: Không khai thì
...
- [x] [`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) §7.3: `game-engine` "port nguyên" → **port có điều kiện** (48 import `@tinimath/shared`, phụ thuộc [`game-template-contract.md`](../specs/01-platform/game-template-contract.md) §11 Q1)
- [x] [`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) §7.3: thêm `packages/config/src/constants.ts` không port
- [x] [`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) §11: đóng **Q5** = GitHub Actions — **superseded 2026-08-06 (lần 2, D-S)**: không CI remote nào, lefthook local. Xem T8b
- [x] [`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) §11: thêm Q10 về chiến lược git (corpus spec root untracked — D-A)
- [x] [`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) §7.1: dòng Lint/format → `ultracite ~6.5.1` preset only + `biome ^2.5.7` CLI thật, ghi rõ không `^7` và không CLI `ultracite check`
...
- [ ] **Task #2** — 16 spec `00-foundation` `draft` → `approved` thì mở khoá `BR-RBS-04`
  ...
  - **51 open question** trong 16 spec foundation ([`repo-bootstrap.md`](../specs/00-foundation/repo-bootstrap.md) 11 · [`mvp-scope.md`](../specs/00-foundation/mvp-scope.md)/[`payment-flow.md`](../specs/00-foundation/payment-flow.md)/[`package-catalog.md`](../specs/00-foundation/package-catalog.md)/[`child-data-compliance.md`](../specs/00-foundation/child-data-compliance.md) 4 mỗi cái) — cần chốt bar approve: OQ nào phải đóng trước `approved`
...
- [ ] Audit `packages/ui` (1.2M) vs [`design-system-contract.md`](../specs/08-quality/design-system-contract.md)
- [ ] Cân nhắc bump PostgreSQL > 17 (§11 Q2)
- [ ] `packages/payment` / `packages/notification`: tách ngay hay inline ([`monorepo-package-architecture.md`](../specs/00-foundation/monorepo-package-architecture.md) §11 Q3)
