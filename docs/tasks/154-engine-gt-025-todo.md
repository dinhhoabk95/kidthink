...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-025` không parse được `content_contract`: ................
- [ ] Số level `GT-025` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-025/template.ts` và `session.ts` (94 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT025`, `engine: spot-difference`, `batch: taxonomy-gap`.
...
- [ ] Viết mới mục 6 — `BR-E025-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E025-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-025` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-025` trong bậc thang.
...
- [ ] Thêm `GT-025` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-025` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP154.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-025`

- [ ] Đo: bao nhiêu trong 4 level trượt, thiếu trường nào.
- [ ] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-025` về **0**.
...
- [ ] 1. Spec `GT-025` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-025` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-025` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-025` = 0.
