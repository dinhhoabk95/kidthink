# Checklist — Task #154: Engine `GT-025` Tìm điểm khác biệt

> Kế hoạch: [`154-engine-gt-025-plan.md`](154-engine-gt-025-plan.md).
> Vị trí **25 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-025`

- [x] `level_count` — kỳ vọng **4**. Đo: **4**.
- [x] `min_band_count` trên 3 band hợp lệ: **3-4:2 4-5:1 5-6:1 min=1**.
- [x] `thinking_span`: **2**.
- [x] `what_span`: **2**.
- [x] `theme_span`: **4**.
- [x] `difficulty_span`: **3**.
- [x] `out_of_band_count`: **0**.
- [x] Số level `GT-025` không parse được `content_contract`: **0**.
- [x] Số level `GT-025` có `access_tier` là `free` hoặc `login`: **2**.
- [x] Đọc `packages/game-engine/src/templates/GT-025/template.ts` và `session.ts` (94 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-025.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT025`, `engine: spot-difference`, `batch: taxonomy-gap`.
...
- [x] Viết mới mục 6 — `BR-E025-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E025-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-025` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-025` trong bậc thang.

## WP154.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-025/`
...
- [x] Thêm `GT-025` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-025` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP154.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-025`

- [x] Đo: bao nhiêu trong 4 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-025` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP154.4 — Level ngoài band
...
- [ ] Mở một màn `GT-025` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-025` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-025` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-025` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-025` parse được contract.
- [x] 4. `out_of_band_count` của `GT-025` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-025`.
- [x] 6. `GT-025` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-025` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=4 band=3-4:2 4-5:1 5-6:1 min=1 thinking=2 what=2 theme=4 diff=3
- `BR-E025-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP154.2 (giờ): ................
