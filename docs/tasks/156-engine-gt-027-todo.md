# Checklist — Task #156: Engine `GT-027` Đổi luật giữa chừng

> Kế hoạch: [`156-engine-gt-027-plan.md`](156-engine-gt-027-plan.md).
> Vị trí **27 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-027`
...
- [x] Số level `GT-027` không parse được `content_contract`: **0**.
- [x] Số level `GT-027` có `access_tier` là `free` hoặc `login`: **0**.
- [x] Đọc `packages/game-engine/src/templates/GT-027/template.ts` và `session.ts` (157 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-027.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT027`, `engine: rule-switch`, `batch: taxonomy-gap`.
...
- [x] Viết mới mục 6 — `BR-E027-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E027-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-027` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-027` trong bậc thang.

## WP156.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-027/`
...
- [x] Thêm `GT-027` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-027` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP156.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-027`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-027` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP156.4 — Level ngoài band

**Cỡ:** S

- [ ] Đo level `GT-027` gắn band bị cấm (`3-4`).
...
- [ ] `out_of_band_count` của `GT-027` về **0**.

## WP156.5 — Nội dung tới sàn bậc 1

**Cỡ:** M · một PR

- [ ] Soạn **3 level** mới cho `GT-027`.
- [ ] `level_count` ≥6.
...
- [ ] Mở một màn `GT-027` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-027` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-027` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-027` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-027` parse được contract.
- [x] 4. `out_of_band_count` của `GT-027` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-027`.
...
- [ ] Cập nhật hàng `GT-027` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=4-5:0 5-6:3 min=0 thinking=1 what=1 theme=1 diff=2
- `BR-E027-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP156.2 (giờ): ................
