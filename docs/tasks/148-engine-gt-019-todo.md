# Checklist — Task #148: Engine `GT-019` Xoay và lật mảnh

> Kế hoạch: [`148-engine-gt-019-plan.md`](148-engine-gt-019-plan.md).
> Vị trí **19 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-019`

- [x] `level_count` — kỳ vọng **3**. Đo: **3**.
- [x] `min_band_count` trên 3 band hợp lệ: **3-4:0 4-5:2 5-6:1 min=0**.
- [x] `thinking_span`: **1**.
- [x] `what_span`: **1**.
- [x] `theme_span`: **2**.
- [x] `difficulty_span`: **3**.
- [x] `out_of_band_count`: **0**.
- [x] Số level `GT-019` không parse được `content_contract`: **0**.
- [x] Số level `GT-019` có `access_tier` là `free` hoặc `login`: **1**.
- [x] Đọc `packages/game-engine/src/templates/GT-019/template.ts` và `session.ts` (224 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-019.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT019`, `engine: rotate-transform`, `batch: legacy-v1`.
...
- [x] Viết mới mục 6 — `BR-E019-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E019-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-019` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-019` trong bậc thang.

## WP148.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-019/`
...
- [x] Thêm `GT-019` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-019` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP148.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-019`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-019` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP148.4 — Level ngoài band
...
- [ ] Mở một màn `GT-019` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-019` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-019` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-019` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-019` parse được contract.
- [x] 4. `out_of_band_count` của `GT-019` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-019`.
- [x] 6. `GT-019` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-019` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=3-4:0 4-5:2 5-6:1 min=0 thinking=1 what=1 theme=2 diff=3
- `BR-E019-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP148.2 (giờ): ................
