# Task #143 — Engine `GT-014` Cân hai bên

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **14 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-014.md`](../specs/01-platform/engines/GT-014.md) — nâng từ phiếu thành
...
> - quyết định `Q116-1` — nguyên thuỷ vẽ mới (đòn cân xoay theo góc)

## 1. Engine này là gì

Trẻ thêm bớt vật lên hai đĩa cân để đạt một mục tiêu: cân bằng, hoặc bên này nặng hơn. Engine này biến so sánh trừu tượng thành **phản hồi vật lý** — đĩa nghiêng ngay khi đặt vật, nên trẻ tự sửa mà không cần ai chấm.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-014/template.ts:53`](../../packages/game-engine/src/templates/GT-014/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-014` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `balance-scale` |
| Lô | `montessori` |
...
| Trường `content_pack` | `left_pan` · `right_pan` · `tray` · `goal` · `target_side` |
| Trường `difficulty_params` | `tray_count` · `weight_span` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `balance-system` |
| `session.ts` | 178 dòng |
...
Vật nặng hơn được vẽ nhỏ hơn vật nhẹ. Parse sạch; sai vì ở lứa 5–6 kích thước nhìn thấy là manh mối mạnh hơn nhãn khối lượng, và bài dạy ngược điều muốn dạy. Nếu cố ý muốn phá liên hệ kích thước với khối lượng thì đó là một bài riêng, phải nói rõ trong `prompt`.

Ca này là lý do engine cần **`BR-E014-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-014/template.ts
grep -c 'template_code: "GT-014"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP143.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-014.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT014`, `engine: balance-scale`,
...
4. **Mục 6 — `BR-E014-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E014-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-014` vào `engine-spec-ready.json`.

### WP143.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-014/`
...
- Vẽ bằng 5 nguyên thuỷ của `RenderSystem` và nguyên thuỷ mới đã chốt ở `Q116-1` (đòn cân xoay theo góc).
- Trạng thái thị giác riêng của engine ghi ở mục 12 của spec.
- Vùng chạm đo ở **3 band hợp lệ**: `3-4`, `4-5`, `5-6`.

Sau khi bảy phép kiểm xanh: thêm `GT-014` vào
`packages/game-engine/config/render-implemented.json` — **một dòng, không sửa dòng khác**.

**Cấm — NEVER** bắt đầu WP này trước khi `Q116-1` có quyết định người: thêm nguyên thuỷ vào `RenderSystem` là quyết định kiến trúc, mục 10 của `engine-render-contract.md` xếp vào *Ask first*.

### WP143.3 — Nợ `content_pack` của engine

**Cỡ:** S · **Ranh giới PR:** `packages/db/src/seed-content`

Cổng 1 sau [`Task #117`](117-seed-gate-truth-plan.md) parse `content_pack` bằng
`content_contract` thật. Đo lại số level `GT-014` trượt, rồi sửa **của riêng engine này**.

1. Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
2. Sửa bằng **version mới**; cấm — NEVER `UPDATE` bản đã publish (`BR-CSA-01`).
3. Con số trượt của `GT-014` về 0.

Nếu đo ra 0 trượt: ghi lại con số và bỏ WP này, kèm lý do.

### WP143.4 — Level ngoài band của engine
...
### WP143.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-014` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-014` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-014` trong bậc thang.
2. `check:render` in `GT-014` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-014` → cổng đỏ.
3. Mọi `content_pack` của `GT-014` parse được `content_contract`.
4. `out_of_band_count` của `GT-014` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-014`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-014` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-014` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q143-1` | `BR-E014-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-014`** mà chưa ai viết? | WP143.1 | Sư phạm + Backend |
| `Q143-2` | Sáu số đo chiều sâu của `GT-014` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP143.5 | Nội dung |
| `Q143-3` | Nguyên thuỷ **đòn cân xoay theo góc** thuộc `RenderSystem` hay thuộc thư mục engine? `BR-ERC-05` cấm vẽ tay ngoài `RenderSystem`, nên để trong engine là phải nới rule — thứ **Cấm — NEVER** | WP143.2 | Backend |
