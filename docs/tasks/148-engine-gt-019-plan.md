# Task #148 — Engine `GT-019` Xoay và lật mảnh

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **19 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-019.md`](../specs/01-platform/engines/GT-019.md) — nâng từ phiếu thành
...
Trẻ xoay hoặc lật một mảnh cho khớp vào ô đích. Engine này dạy **hình không đổi khi xoay** — một hằng số nhận thức mà trẻ mầm non chưa có sẵn: cùng một tam giác quay 90 độ vẫn bị nhiều trẻ đọc là hình khác.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-019/template.ts:51`](../../packages/game-engine/src/templates/GT-019/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-019` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `rotate-transform` |
| Lô | `legacy-v1` |
| Layout khai trong template | `grid` · `flex-wrap` · `top-source-bottom-target` · `left-source-right-target` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `target_slots` · `pieces` |
| Trường `difficulty_params` | `allow_flip` · `rotation_step` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Không |
| System và mechanic | `placement-mechanic` · `rotation-system` |
| `session.ts` | 224 dòng |
...
Mảnh có tính đối xứng quay: hình vuông trơn khớp ở mọi góc. Parse sạch; sai vì không có phép xoay nào cần thiết, và trẻ thả bừa là đúng. Mảnh của engine này phải **không đối xứng** theo phép xoay đang dùng.

Ca này là lý do engine cần **`BR-E019-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-019/template.ts
grep -c 'template_code: "GT-019"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP148.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-019.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT019`, `engine: rotate-transform`,
...
4. **Mục 6 — `BR-E019-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E019-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-019` vào `engine-spec-ready.json`.

### WP148.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-019/`
...
- Slot lấy từ `grid` · `flex-wrap` · `top-source-bottom-target` · `left-source-right-target` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-019` vào
...
### WP148.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-019` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-019` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-019` trong bậc thang.
2. `check:render` in `GT-019` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-019` → cổng đỏ.
3. Mọi `content_pack` của `GT-019` parse được `content_contract`.
4. `out_of_band_count` của `GT-019` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-019`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-019` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-019` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q148-1` | `BR-E019-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-019`** mà chưa ai viết? | WP148.1 | Sư phạm + Backend |
| `Q148-2` | Sáu số đo chiều sâu của `GT-019` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP148.5 | Nội dung |
