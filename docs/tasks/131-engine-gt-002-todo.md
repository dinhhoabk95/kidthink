...
- [ ] `level_count` — kỳ vọng **27**.
- [ ] `min_band_count` trên 2 band hợp lệ (`4-5`, `5-6`): ................
- [ ] `thinking_span`: ................
- [ ] `what_span`: ................
- [ ] `theme_span`: ................
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-002` không parse được `content_contract`: ................
- [ ] Số level `GT-002` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-002/template.ts` và `session.ts` (57 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT002`, `engine: tap-select-multi`, `batch: mvp`.
...
- [ ] Viết mới mục 6 — `BR-E002-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E002-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-002` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-002` trong bậc thang.
...
- [ ] Phép kiểm 2 — vùng chạm ≥ sàn chạm, đo ở 2 band hợp lệ (`4-5`, `5-6`) (`BR-ERC-04`).
...
- [ ] Thêm `GT-002` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-002` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP131.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-002`

- [ ] Đo: bao nhiêu trong 27 level trượt, thiếu trường nào.
...
- [ ] 1. Spec `GT-002` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-002` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
