# Checklist — Task #131: Engine `GT-002` Chọn nhiều đáp án

> Kế hoạch: [`131-engine-gt-002-plan.md`](131-engine-gt-002-plan.md).
> Vị trí **2 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-002`

- [x] `level_count` — kỳ vọng **27**. Đo: **27**.
- [x] `min_band_count` trên 2 band hợp lệ (`4-5`, `5-6`): ................
- [x] `thinking_span`: **7**.
- [x] `what_span`: **8**.
- [x] `theme_span`: **5**.
- [x] `difficulty_span`: **4**.
- [x] `out_of_band_count`: **12**.
- [x] Số level `GT-002` không parse được `content_contract`: **27**.
- [x] Số level `GT-002` có `access_tier` là `free` hoặc `login`: **11**.
- [x] Đọc `packages/game-engine/src/templates/GT-002/template.ts` và `session.ts` (57 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-002.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT002`, `engine: tap-select-multi`, `batch: mvp`.
...
- [x] Viết mới mục 6 — `BR-E002-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E002-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-002` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-002` trong bậc thang.

## WP131.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-002/`
...
- [x] Thêm `GT-002` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-002` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP131.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-002`

- [x] Đo: bao nhiêu trong 27 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-002` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP131.4 — Level ngoài band

**Cỡ:** S

- [ ] Đo level `GT-002` gắn band bị cấm (`3-4`).
...
- [ ] `out_of_band_count` của `GT-002` về **0**.
...
- [ ] Mở một màn `GT-002` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-002` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-002` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-002` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-002` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-002` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-002`.
- [x] 6. `GT-002` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-002` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=27 band=4-5:8 5-6:7 min=7 thinking=7 what=8 theme=5 diff=4
- `BR-E002-*` đã viết: ................
- Số level trượt parse, trước và sau: **27**
- Số level đã soạn thêm: ................
- Chi phí thật của WP131.2 (giờ): ................
