# Checklist — Task #149: Engine `GT-020` Lật thẻ tìm cặp

> Kế hoạch: [`149-engine-gt-020-plan.md`](149-engine-gt-020-plan.md).
> Vị trí **20 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-020`
...
- [x] Số level `GT-020` không parse được `content_contract`: **0**.
- [x] Số level `GT-020` có `access_tier` là `free` hoặc `login`: **2**.
- [x] Đọc `packages/game-engine/src/templates/GT-020/template.ts` và `session.ts` (121 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-020.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT020`, `engine: memory-flip`, `batch: legacy-v1`.
...
- [x] Viết mới mục 6 — `BR-E020-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E020-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-020` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-020` trong bậc thang.

## WP149.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-020/`
...
- [x] Thêm `GT-020` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-020` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP149.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-020`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-020` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP149.4 — Level ngoài band
...
- [ ] Mở một màn `GT-020` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-020` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-020` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-020` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-020` parse được contract.
- [x] 4. `out_of_band_count` của `GT-020` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-020`.
- [x] 6. `GT-020` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-020` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
