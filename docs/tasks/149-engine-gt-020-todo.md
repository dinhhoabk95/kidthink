...
- [ ] `difficulty_span`: ................
- [ ] `out_of_band_count`: ................
- [ ] Số level `GT-020` không parse được `content_contract`: ................
- [ ] Số level `GT-020` có `access_tier` là `free` hoặc `login`: ................
- [ ] Đọc `packages/game-engine/src/templates/GT-020/template.ts` và `session.ts` (121 dòng).
...
- [ ] Frontmatter đủ 9 trường; `spec: ENGINE-GT020`, `engine: memory-flip`, `batch: legacy-v1`.
...
- [ ] Viết mới mục 6 — `BR-E020-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [ ] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [ ] Viết mới mục 9 — Gherkin, mỗi `BR-E020-*` ≥1 scenario (`BR-ESS-13`).
...
- [ ] Thêm `GT-020` vào `engine-spec-ready.json` — **một dòng**.
- [ ] `check:engine-specs` xanh với `GT-020` trong bậc thang.
...
- [ ] Thêm `GT-020` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [ ] Ca âm: xoá `render()` của `GT-020` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.
...
- [ ] 1. Spec `GT-020` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [ ] 2. `check:render` in `GT-020` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-020` parse được contract.
- [ ] 4. `out_of_band_count` của `GT-020` = 0.
