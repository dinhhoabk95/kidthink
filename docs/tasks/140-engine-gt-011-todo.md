# Checklist — Task #140: Engine `GT-011` Ma trận chọn hình

> Kế hoạch: [`140-engine-gt-011-plan.md`](140-engine-gt-011-plan.md).
> Vị trí **11 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-011`
...
- [x] Số level `GT-011` không parse được `content_contract`: **1**.
- [x] Số level `GT-011` có `access_tier` là `free` hoặc `login`: **1**.
- [x] Đọc `packages/game-engine/src/templates/GT-011/template.ts` và `session.ts` (102 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-011.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT011`, `engine: matrix-choice`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E011-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E011-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-011` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-011` trong bậc thang.

## WP140.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-011/`
...
- [x] Thêm `GT-011` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-011` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP140.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-011`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-011` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP140.4 — Level ngoài band
...
- [ ] Mở một màn `GT-011` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-011` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-011` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-011` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-011` parse được contract.
- [x] 4. `out_of_band_count` của `GT-011` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-011`.
- [x] 6. `GT-011` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-011` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
