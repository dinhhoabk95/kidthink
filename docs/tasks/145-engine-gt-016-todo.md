# Checklist — Task #145: Engine `GT-016` Xoay kim đồng hồ

> Kế hoạch: [`145-engine-gt-016-plan.md`](145-engine-gt-016-plan.md).
> Vị trí **16 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
> `Q116-1` (nguyên thuỷ mặt đồng hồ và kim xoay) phải có quyết định người **trước** WP145.2.
>
...
## Preflight — đo sáu số đo của `GT-016`
...
- [x] Số level `GT-016` không parse được `content_contract`: **1**.
- [x] Số level `GT-016` có `access_tier` là `free` hoặc `login`: **1**.
- [x] Đọc `packages/game-engine/src/templates/GT-016/template.ts` và `session.ts` (193 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-016.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT016`, `engine: clock-hands`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E016-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E016-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-016` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-016` trong bậc thang.

## WP145.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-016/`

- [ ] `Q116-1` đã có quyết định người về nguyên thuỷ **mặt đồng hồ và kim xoay**.
...
- [x] Thêm `GT-016` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-016` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP145.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-016`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [ ] Số trượt của `GT-016` về **0**.
- [ ] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP145.4 — Level ngoài band
...
- [ ] Mở một màn `GT-016` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-016` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-016` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-016` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [ ] 3. Mọi `content_pack` của `GT-016` parse được contract.
- [x] 4. `out_of_band_count` của `GT-016` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-016`.
- [x] 6. `GT-016` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-016` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=3-4:0 4-5:0 5-6:3 min=0 thinking=1 what=1 theme=2 diff=3
- `BR-E016-*` đã viết: ................
- Số level trượt parse, trước và sau: **1**
- Số level đã soạn thêm: ................
- Chi phí thật của WP145.2 (giờ): ................
