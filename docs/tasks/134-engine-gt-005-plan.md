# Task #134 — Engine `GT-005` Ghép cặp

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **3 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-005.md`](../specs/01-platform/engines/GT-005.md) — nâng từ phiếu thành
...
Trẻ nối 2–6 cặp giữa hai tập. Cơ chế này dạy **tương ứng một–một**, khái niệm nền của phép đếm: mỗi cái bên này ứng với đúng một cái bên kia, không thừa không thiếu. Nó cũng là engine rẻ nhất để dạy quan hệ ký hiệu — số với lượng, chữ với hình, âm với vật.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-005/template.ts:34`](../../packages/game-engine/src/templates/GT-005/template.ts) — `BR-ESS-02` buộc spec ghi kèm
nguồn dòng này và cổng so lại với registry lúc chạy.

Lô `mvp` (MVP). Hôm nay engine có **24 level** trong corpus seed và
**chưa cài `render()`** — trẻ mở một màn `GT-005` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `pair-match` |
| Lô | `mvp` |
| Layout khai trong template | `two-column-matching` · `card-flip-grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `pairs` |
| Trường `difficulty_params` | `hint_after_ms` · `allow_retry` · `shuffle_sides` |
| Dùng `.refine()` | Không |
| System và mechanic | `pairing-mechanic` |
| `session.ts` | 81 dòng |
| Level trong corpus | **24** |
...
Sáu cặp ở band 3-4, mỗi cặp là một loài chim khác nhau. Parse sạch. Sai vì phân biệt sáu loài chim là bài học riêng chưa dạy; trẻ ghép ngẫu nhiên và đúng do may.

Ca này là lý do engine cần **`BR-E005-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-005/template.ts
grep -c 'template_code: "GT-005"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP134.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-005.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT005`, `engine: pair-match`,
...
4. **Mục 6 — `BR-E005-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E005-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-005` vào `engine-spec-ready.json`.

### WP134.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-005/`
...
- Slot lấy từ `two-column-matching` · `card-flip-grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-005` vào
...
### WP134.5 — Nội dung tới sàn bậc 1
...
### WP134.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-005` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-005` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-005` trong bậc thang.
2. `check:render` in `GT-005` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-005` → cổng đỏ.
3. Mọi `content_pack` của `GT-005` parse được `content_contract`.
4. `out_of_band_count` của `GT-005` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-005`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-005` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-005` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q134-1` | `BR-E005-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-005`** mà chưa ai viết? | WP134.1 | Sư phạm + Backend |
| `Q134-2` | Sáu số đo chiều sâu của `GT-005` hôm nay là bao nhiêu? Chỉ `level_count` = 24 là chắc; năm số còn lại phải đo ở Preflight | WP134.5 | Nội dung |
