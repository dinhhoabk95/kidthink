# Task #130 — Engine `GT-001` Chọn một đáp án

> **Loại task:** lát dọc engine (S) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **1 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-001.md`](../specs/01-platform/engines/GT-001.md) — nâng từ phiếu thành
...
Trẻ nghe một câu hỏi, nhìn 2–6 lựa chọn, chạm vào **một**. Đây là engine có rào cản vận động thấp nhất trong corpus: một cú chạm, không kéo, không giữ. Vì vậy nó là cửa vào của band 3–4 và là engine gánh nhiều nội dung nhất hôm nay (37 level).

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-001/template.ts:33`](../../packages/game-engine/src/templates/GT-001/template.ts) — `BR-ESS-02` buộc spec ghi kèm
nguồn dòng này và cổng so lại với registry lúc chạy.

Lô `mvp` (MVP). Hôm nay engine có **38 level** trong corpus seed và
**chưa cài `render()`** — trẻ mở một màn `GT-001` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `tap-select` |
| Lô | `mvp` |
...
`content_pack` với `item_count` = 6, band `3-4`, sáu emoji cùng họ (sáu loại quả đỏ). Parse sạch, `limits` sạch. Sai vì trẻ 3 tuổi phải giữ tiêu chí trong trí nhớ làm việc trong khi quét sáu vật gần giống nhau — bài kiểm tra chú ý, không phải bài học nội dung.

Ca này là lý do engine cần **`BR-E001-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-001/template.ts
grep -c 'template_code: "GT-001"' -r packages/db/src/seed-content
...
**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-001.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT001`, `engine: tap-select`,
...
4. **Mục 6 — `BR-E001-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E001-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-001` vào `engine-spec-ready.json`.

### WP130.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-001/`
...
Sau khi bảy phép kiểm xanh: thêm `GT-001` vào
...
`content_contract` thật. Đo lại số level `GT-001` trượt, rồi sửa **của riêng engine này**.

1. Đo: bao nhiêu trong 38 level trượt, thiếu trường nào.
2. Sửa bằng **version mới**; cấm — NEVER `UPDATE` bản đã publish (`BR-CSA-01`).
3. Con số trượt của `GT-001` về 0.

Nếu đo ra 0 trượt: ghi lại con số và bỏ WP này, kèm lý do.

### WP130.4 — Level ngoài band của engine
...
1. Mở một màn `GT-001` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-001` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-001` trong bậc thang.
2. `check:render` in `GT-001` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-001` → cổng đỏ.
3. Mọi `content_pack` của `GT-001` parse được `content_contract`.
4. `out_of_band_count` của `GT-001` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-001`: `level_count` ≥6 và bốn `*_span` ≥2.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q130-1` | `BR-E001-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-001`** mà chưa ai viết? | WP130.1 | Sư phạm + Backend |
| `Q130-2` | Sáu số đo chiều sâu của `GT-001` hôm nay là bao nhiêu? Chỉ `level_count` = 38 là chắc; năm số còn lại phải đo ở Preflight | WP130.5 | Nội dung |
