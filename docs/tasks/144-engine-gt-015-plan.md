# Task #144 — Engine `GT-015` Lưới không lặp

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **15 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-015.md`](../specs/01-platform/engines/GT-015.md) — nâng từ phiếu thành
...
Trẻ điền ký hiệu vào lưới 2x2 hoặc 3x3 sao cho không hàng nào, không cột nào lặp ký hiệu. Engine này dạy **ràng buộc**: một ô không quyết định riêng lẻ mà bị chặn bởi hàng và cột của nó. Đó là bước đầu của suy luận có điều kiện.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-015/template.ts:102`](../../packages/game-engine/src/templates/GT-015/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-015` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `sudoku-mini` |
| Lô | `montessori` |
| Layout khai trong template | `matrix-slot-grid` · `grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `grid_size` · `symbols` · `cells` · `regions` |
| Trường `difficulty_params` | `blank_count` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `constraint-system` |
| `session.ts` | 203 dòng |
...
Lưới 3x3 với 4 ô trống có **hai** lời giải khác nhau. Parse sạch; sai vì trẻ điền một lời giải hợp lệ và bị chấm sai. Mỗi lưới phải có đúng một lời giải, và điều đó phải kiểm bằng bộ giải trước khi seed.

Ca này là lý do engine cần **`BR-E015-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-015/template.ts
grep -c 'template_code: "GT-015"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP144.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-015.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT015`, `engine: sudoku-mini`,
...
4. **Mục 6 — `BR-E015-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E015-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-015` vào `engine-spec-ready.json`.

### WP144.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-015/`
...
- Slot lấy từ `matrix-slot-grid` · `grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-015` vào
...
### WP144.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-015` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-015` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-015` trong bậc thang.
2. `check:render` in `GT-015` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-015` → cổng đỏ.
3. Mọi `content_pack` của `GT-015` parse được `content_contract`.
4. `out_of_band_count` của `GT-015` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-015`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-015` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-015` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q144-1` | `BR-E015-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-015`** mà chưa ai viết? | WP144.1 | Sư phạm + Backend |
| `Q144-2` | Sáu số đo chiều sâu của `GT-015` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP144.5 | Nội dung |
