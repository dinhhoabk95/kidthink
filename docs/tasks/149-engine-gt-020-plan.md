# Task #149 — Engine `GT-020` Lật thẻ tìm cặp

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **20 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-020.md`](../specs/01-platform/engines/GT-020.md) — nâng từ phiếu thành
...
Trẻ lật hai thẻ úp một lượt, nhớ vị trí, và tìm đủ các cặp. Khác `GT-005` ghép cặp ở một điểm quyết định: ở đây **thông tin biến mất** khi thẻ úp lại, nên bài là bài trí nhớ vị trí chứ không phải bài nhận ra quan hệ.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-020/template.ts:35`](../../packages/game-engine/src/templates/GT-020/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-020` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `memory-flip` |
| Lô | `legacy-v1` |
| Layout khai trong template | `card-flip-grid` · `grid` |
| Band engine **cấm** | — |
...
| Trường `difficulty_params` | `flip_back_delay_ms` · `peek_all_initial_ms` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Không |
| System và mechanic | `card-system` · `pairing-mechanic` |
| `session.ts` | 121 dòng |
...
Sáu cặp ở band 3-4 với `peek_all_initial_ms` = 0. Parse sạch, trong `limits`. Sai vì trí nhớ vị trí ở lứa 3–4 giữ được hai đến ba vị trí; mười hai thẻ úp là bài lật ngẫu nhiên kéo dài.

Ca này là lý do engine cần **`BR-E020-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-020/template.ts
grep -c 'template_code: "GT-020"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP149.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-020.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT020`, `engine: memory-flip`,
...
4. **Mục 6 — `BR-E020-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E020-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-020` vào `engine-spec-ready.json`.

### WP149.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-020/`
...
- Slot lấy từ `card-flip-grid` · `grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-020` vào
...
### WP149.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-020` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-020` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-020` trong bậc thang.
2. `check:render` in `GT-020` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-020` → cổng đỏ.
3. Mọi `content_pack` của `GT-020` parse được `content_contract`.
4. `out_of_band_count` của `GT-020` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-020`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-020` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-020` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q149-1` | `BR-E020-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-020`** mà chưa ai viết? | WP149.1 | Sư phạm + Backend |
| `Q149-2` | Sáu số đo chiều sâu của `GT-020` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP149.5 | Nội dung |
