# Checklist — Task #137: Engine `GT-008` Kéo vào ô chứa

> Kế hoạch: [`137-engine-gt-008-plan.md`](137-engine-gt-008-plan.md).
> Vị trí **8 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-008`

- [x] `level_count` — kỳ vọng **6**. Đo: **6**.
- [x] `min_band_count` trên 3 band hợp lệ: **3-4:2 4-5:2 5-6:2 min=2**.
- [x] `thinking_span`: **4**.
- [x] `what_span`: **3**.
- [x] `theme_span`: **4**.
- [x] `difficulty_span`: **3**.
- [x] `out_of_band_count`: **0**.
- [x] Số level `GT-008` không parse được `content_contract`: **0**.
- [x] Số level `GT-008` có `access_tier` là `free` hoặc `login`: **2**.
- [x] Đọc `packages/game-engine/src/templates/GT-008/template.ts` và `session.ts` (97 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-008.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT008`, `engine: drag-to-slot`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E008-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E008-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-008` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-008` trong bậc thang.

## WP137.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-008/`
...
- [x] Thêm `GT-008` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-008` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP137.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-008`

- [x] Đo: bao nhiêu trong 6 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-008` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP137.4 — Level ngoài band
...
- [ ] Mở một màn `GT-008` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-008` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-008` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-008` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-008` parse được contract.
- [x] 4. `out_of_band_count` của `GT-008` = 0.
- [x] 5. `check:engine-depth` bậc 1 xanh cho `GT-008`.
- [x] 6. `GT-008` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-008` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=6 band=3-4:2 4-5:2 5-6:2 min=2 thinking=4 what=3 theme=4 diff=3
- `BR-E008-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP137.2 (giờ): ................
