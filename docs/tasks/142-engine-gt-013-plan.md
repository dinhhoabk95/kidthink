# Task #142 — Engine `GT-013` Tìm đường mê cung

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **13 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-013.md`](../specs/01-platform/engines/GT-013.md) — nâng từ phiếu thành
...
> - quyết định `Q116-1` — nguyên thuỷ vẽ mới (lưới và tường mê cung)

## 1. Engine này là gì

Trẻ vạch một đường từ điểm đầu tới đích qua lưới, đi qua các ô bắt buộc và tránh ngõ cụt. Engine này là nơi rõ nhất trong corpus để dạy **lập kế hoạch trước khi hành động**: nhìn trước vài bước rồi mới chạm.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-013/template.ts:110`](../../packages/game-engine/src/templates/GT-013/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-013` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `maze-route` |
| Lô | `montessori` |
...
| `asset_kinds` | `audio` |
| Trường `content_pack` | `grid` · `required_cells` · `input_mode` |
| Trường `difficulty_params` | `dead_end_count` · `required_cell_count` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Có — contract có `.refine()`, nên parse chặt hơn phần lớn engine khác |
| System và mechanic | `maze-system` · `ordering-mechanic` |
| `session.ts` | 203 dòng |
...
Lưới có hai đường đi hợp lệ tới đích, và chỉ một đường đi qua đủ `required_cells`. Parse sạch; sai vì trẻ tìm ra đường ngắn hơn, tới đích, và bị chấm sai. Hoặc lưới chỉ có một đường, hoặc `prompt` phải nói rõ điều kiện đi qua.

Ca này là lý do engine cần **`BR-E013-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-013/template.ts
grep -c 'template_code: "GT-013"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP142.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-013.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT013`, `engine: maze-route`,
...
4. **Mục 6 — `BR-E013-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E013-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-013` vào `engine-spec-ready.json`.

### WP142.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-013/`
...
- Vẽ bằng 5 nguyên thuỷ của `RenderSystem` và nguyên thuỷ mới đã chốt ở `Q116-1` (lưới và tường mê cung).
- Trạng thái thị giác riêng của engine ghi ở mục 12 của spec.
- Vùng chạm đo ở **3 band hợp lệ**: `3-4`, `4-5`, `5-6`.

Sau khi bảy phép kiểm xanh: thêm `GT-013` vào
...
### WP142.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-013` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-013` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-013` trong bậc thang.
2. `check:render` in `GT-013` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-013` → cổng đỏ.
3. Mọi `content_pack` của `GT-013` parse được `content_contract`.
4. `out_of_band_count` của `GT-013` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-013`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-013` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-013` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q142-1` | `BR-E013-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-013`** mà chưa ai viết? | WP142.1 | Sư phạm + Backend |
| `Q142-2` | Sáu số đo chiều sâu của `GT-013` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP142.5 | Nội dung |
| `Q142-3` | Nguyên thuỷ **lưới và tường mê cung** thuộc `RenderSystem` hay thuộc thư mục engine? `BR-ERC-05` cấm vẽ tay ngoài `RenderSystem`, nên để trong engine là phải nới rule — thứ **Cấm — NEVER** | WP142.2 | Backend |
