# Checklist — Task #151: Engine `GT-022` Tìm vật thể ẩn

> Kế hoạch: [`151-engine-gt-022-plan.md`](151-engine-gt-022-plan.md).
> Vị trí **22 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
## Preflight — đo sáu số đo của `GT-022`
...
- [x] Số level `GT-022` không parse được `content_contract`: **0**.
- [x] Số level `GT-022` có `access_tier` là `free` hoặc `login`: **0**.
- [x] Đọc `packages/game-engine/src/templates/GT-022/template.ts` và `session.ts` (131 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-022.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT022`, `engine: hidden-object`, `batch: legacy-v1`.
...
- [x] Viết mới mục 6 — `BR-E022-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E022-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-022` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-022` trong bậc thang.

## WP151.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-022/`
...
- [x] Thêm `GT-022` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-022` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP151.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-022`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-022` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP151.4 — Level ngoài band
...
- [ ] Mở một màn `GT-022` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-022` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-022` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-022` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-022` parse được contract.
- [x] 4. `out_of_band_count` của `GT-022` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-022`.
...
- [ ] Cập nhật hàng `GT-022` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=3-4:0 4-5:1 5-6:2 min=0 thinking=1 what=1 theme=2 diff=2
- `BR-E022-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP151.2 (giờ): ................
