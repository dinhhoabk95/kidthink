# Task #152 — Engine `GT-023` Lắp ghép hình thể

> **Loại task:** lát dọc engine (M) — một trong 27 task của
> [`Task #116`](116-engine-vertical-slices-plan.md). Vị trí **23 / 27** trong thứ tự chương trình.
> **Spec sở hữu:** [`engines/GT-023.md`](../specs/01-platform/engines/GT-023.md) — nâng từ phiếu thành
...
Trẻ ghép nhiều mảnh vào các mỏ neo để dựng thành một hình mẫu. Khác `GT-019` xoay và lật ở chỗ đây là bài **nhiều mảnh cùng lúc**: trẻ phải nhìn hình tổng thể, chia nó thành phần, và quyết định thứ tự ghép.

**Nguồn dòng cho mục 15 của spec.** Bảng `layouts` · `limits` · `banned_age_bands` ·
`asset_kinds` trích từ [`GT-023/template.ts:46`](../../packages/game-engine/src/templates/GT-023/template.ts) — `BR-ESS-02` buộc spec ghi kèm
...
**chưa cài `render()`** — trẻ mở một màn `GT-023` ra thì thấy canvas trống.

## 2. Bằng chứng đã đo (2026-08-29)

| Số đo | Giá trị |
|---|---|
| Slug engine | `construct` |
| Lô | `legacy-v1` |
| Layout khai trong template | `top-source-bottom-target` · `grid` |
| Band engine **cấm** | — |
...
| Trường `content_pack` | `target_model` · `anchors` · `parts` |
| Trường `difficulty_params` | `snap_radius_px` · `show_anchor_outline` · `hint_after_ms` · `allow_retry` |
| Dùng `.refine()` | Không |
| System và mechanic | `assembly-system` · `placement-mechanic` |
| `session.ts` | 119 dòng |
...
`show_anchor_outline` tắt với 6 mảnh ở band 4-5. Parse sạch; sai vì không có mỏ neo nhìn thấy thì bài trở thành bài thử mọi vị trí, và trẻ bỏ cuộc trước khi hình thành xong.

Ca này là lý do engine cần **`BR-E023-*` riêng**: contract Zod chặn được sai schema, nó
...
sed -n '1,40p' packages/game-engine/src/templates/GT-023/template.ts
grep -c 'template_code: "GT-023"' -r packages/db/src/seed-content
pnpm --filter @mindkid/game-engine check:render
```

## 3. Work package

### WP152.1 — Nâng phiếu thành spec đủ khuôn SDD

**Cỡ:** S · **Ranh giới PR:** `docs/specs/01-platform/engines/GT-023.md`
...
1. Frontmatter đủ 9 trường. `spec: ENGINE-GT023`, `engine: construct`,
...
4. **Mục 6 — `BR-E023-01` trở đi.** Ít nhất một rule sinh từ ca sai ở mục 2.1 trên. Mỗi rule
   kèm lý do. Cấm — NEVER nói lại `BR-GTC-*`, `BR-ERC-*`, `BR-ECD-*`.
5. **Mục 9 — Gherkin.** Mỗi `BR-E023-*` ít nhất một scenario.
6. Mục 15 — trường trích từ registry kèm nguồn dòng: `layouts`, `limits`, `banned_age_bands`,
   `asset_kinds`.
7. `status: draft` → `approved`. Thêm `GT-023` vào `engine-spec-ready.json`.

### WP152.2 — Cài `render()`

**Cỡ:** M · **Ranh giới PR:** `packages/game-engine/src/templates/GT-023/`
...
- Slot lấy từ `top-source-bottom-target` · `grid` qua `resolveLayout()` — cấm — NEVER hằng số toạ độ.
...
Sau khi bảy phép kiểm xanh: thêm `GT-023` vào
...
### WP152.6 — Mở màn thật và đóng spec

**Cỡ:** S

1. Mở một màn `GT-023` thật trong `apps/web`, **nhìn thấy hình**, đính ảnh chụp vào PR.
...
1. Spec `GT-023` đủ khuôn 16 mục; `check:engine-specs` xanh với `GT-023` trong bậc thang.
2. `check:render` in `GT-023` đã cài; bảy phép kiểm vẽ xanh; xoá `render()` của `GT-023` → cổng đỏ.
3. Mọi `content_pack` của `GT-023` parse được `content_contract`.
4. `out_of_band_count` của `GT-023` bằng 0.
5. `check:engine-depth` bậc 1 xanh cho `GT-023`: `level_count` ≥6 và bốn `*_span` ≥2.
6. `GT-023` có ≥1 level `free` hoặc `login`.
7. Ảnh chụp màn `GT-023` thật trong PR.
...
| # | Câu hỏi | Chặn gì | Chủ |
|---|---|---|---|
| `Q152-1` | `BR-E023-*` gồm những rule nào? Ca sai ở mục 2.1 cho ít nhất một; còn rule nào **chỉ đúng với `GT-023`** mà chưa ai viết? | WP152.1 | Sư phạm + Backend |
| `Q152-2` | Sáu số đo chiều sâu của `GT-023` hôm nay là bao nhiêu? Chỉ `level_count` = 3 là chắc; năm số còn lại phải đo ở Preflight | WP152.5 | Nội dung |
