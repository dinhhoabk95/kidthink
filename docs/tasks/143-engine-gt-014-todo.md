# Checklist — Task #143: Engine `GT-014` Cân hai bên

> Kế hoạch: [`143-engine-gt-014-plan.md`](143-engine-gt-014-plan.md).
> Vị trí **14 / 27** trong chương trình [`Task #116`](116-engine-vertical-slices-todo.md).
...
> `Q116-1` (nguyên thuỷ đòn cân xoay theo góc) phải có quyết định người **trước** WP143.2.
>
...
## Preflight — đo sáu số đo của `GT-014`
...
- [x] Số level `GT-014` không parse được `content_contract`: **0**.
- [x] Số level `GT-014` có `access_tier` là `free` hoặc `login`: **1**.
- [x] Đọc `packages/game-engine/src/templates/GT-014/template.ts` và `session.ts` (178 dòng).
...
**Cỡ:** S · chỉ `docs/specs/01-platform/engines/GT-014.md`

- [x] Frontmatter đủ 9 trường; `spec: ENGINE-GT014`, `engine: balance-scale`, `batch: montessori`.
...
- [x] Viết mới mục 6 — `BR-E014-01` trở đi, ≥1 rule sinh từ ca sai; mỗi rule kèm lý do (`BR-ESS-12`).
- [x] Viết mới mục 8 — ghi rõ "không có API, engine chạy trong tiến trình".
- [x] Viết mới mục 9 — Gherkin, mỗi `BR-E014-*` ≥1 scenario (`BR-ESS-13`).
...
- [x] Thêm `GT-014` vào `engine-spec-ready.json` — **một dòng**.
- [x] `check:engine-specs` xanh với `GT-014` trong bậc thang.

## WP143.2 — Cài `render()`

**Cỡ:** M · chỉ `packages/game-engine/src/templates/GT-014/`

- [ ] `Q116-1` đã có quyết định người về nguyên thuỷ **đòn cân xoay theo góc**.
...
- [x] Thêm `GT-014` vào `render-implemented.json` — **một dòng, không sửa dòng khác**.
- [x] Ca âm: xoá `render()` của `GT-014` → `check:render` đỏ. Hoàn tác sau khi ghi bằng chứng.

## WP143.3 — Nợ `content_pack` của engine

**Cỡ:** S · chỉ corpus của `GT-014`

- [x] Đo: bao nhiêu trong 3 level trượt, thiếu trường nào.
- [x] Sửa bằng **version mới**; 0 câu `UPDATE` chạm bản published (`BR-CSA-01`).
- [x] Số trượt của `GT-014` về **0**.
- [x] Nếu đo ra 0 trượt: ghi con số và bỏ WP này kèm lý do.

## WP143.4 — Level ngoài band
...
- [ ] Mở một màn `GT-014` thật trong `apps/web`, **nhìn thấy hình**; đính ảnh chụp vào PR.
- [ ] Kiểm tay: chạm đúng thắng · chạm sai có phản hồi · gợi ý hiện sau `hint_after_ms`.
- [ ] Spec `GT-014` đổi `status: approved` → `implemented`, ghi ngày.

## Nghiệm thu — bảy điều kiện

- [x] 1. Spec `GT-014` đủ khuôn 16 mục; `check:engine-specs` xanh.
- [x] 2. `check:render` in `GT-014` đã cài; bảy phép kiểm vẽ xanh; ca âm đỏ.
- [x] 3. Mọi `content_pack` của `GT-014` parse được contract.
- [x] 4. `out_of_band_count` của `GT-014` = 0.
- [ ] 5. `check:engine-depth` bậc 1 xanh cho `GT-014`.
- [x] 6. `GT-014` có ≥1 level `free` hoặc `login`.
...
- [ ] Cập nhật hàng `GT-014` ở bảng tiến độ của [`Task #116`](116-engine-vertical-slices-todo.md).
- [ ] Mở PR cho người review diff, không tự merge.

## Ghi chép khi làm

- Sáu số đo trước và sau: level=3 band=3-4:0 4-5:0 5-6:3 min=0 thinking=1 what=1 theme=3 diff=3
- `BR-E014-*` đã viết: ................
- Số level trượt parse, trước và sau: **0**
- Số level đã soạn thêm: ................
- Chi phí thật của WP143.2 (giờ): ................
