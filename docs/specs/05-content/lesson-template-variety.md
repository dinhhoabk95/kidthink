...
Và chỗ trống đó không phải chuyện lý thuyết. Đo ngày 2026-08-22 trên corpus seed: trong **81**
activity đã soạn, số activity `kind: digital_game` là **0**. Phân bố thực tế là
`manipulative` 28, `home_activity` 12, `movement` 9, `discussion` 8, `storytelling` 6,
`observation` 6, `mini_project` 6, `assessment` 6. Chuỗi
`lessons` sang `lesson_activities` sang `activities` sang `game_levels` sang `game_templates`
tồn tại trong schema và chạy được ở bề mặt chơi, nhưng **chưa hàng seed nào đi qua nó**. Nói
cách khác, hôm nay không bài học nào dẫn tới một màn chơi, nên câu "một bài học, nhiều khuôn"
chưa có dữ liệu để đúng hay sai.

File này đặt luật đó và cấp cho nó một cổng đếm được.
...
| ID | Rule | Vì sao |
|---|---|---|
| `BR-LTV-01` (mỗi bài học có màn chơi) | Mỗi lesson `published` có **≥1** activity `kind: digital_game` trỏ vào game level đã `published` | Hôm nay con số đó là 0 trên 81. Một bài học không dẫn tới màn chơi nào thì phần số của sản phẩm không tồn tại với người dùng bài học đó |
...
| `BR-LTV-04` (bước chơi phục vụ bài học) | Mọi bước chơi số của một bài học trỏ vào game level có skill thuộc cùm kỹ năng của bài học | `BR-LSM-08` — một bài học phục vụ một cùm mục tiêu. Nhét một màn chơi lạ vào cho đủ số là làm hỏng cả hai |
...
Bậc 3 là chỗ duy nhất `kind: digital_game` xuất hiện, và cũng là chỗ corpus đang trống.
