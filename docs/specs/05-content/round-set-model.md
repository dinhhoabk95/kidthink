...
|---|---|---|---|---|---|
| 1 | `game_level_rounds` là bảng con hay cột `rounds` jsonb trên `game_levels`? Bảng con cho phép ràng buộc UNIQUE và query từng vòng; jsonb rẻ hơn và khớp cách `content_pack` đang lưu | Hình dạng schema, migration | P2 | Backend | Chờ chủ [`schema-content-taxonomy.md`](../01-platform/schema-content-taxonomy.md). Spec này viết theo phương án bảng con; đổi sang jsonb chỉ đổi mục 7.2, không đổi mục 6 |
| 2 | Migrate level đã seed: copy `content_pack` hiện có thành `round_index = 0` rồi drop cột cũ theo expand-contract, hay giữ cột cũ vĩnh viễn làm vòng 0? | Kế hoạch migration | P2 | người quyết | Chờ. Repo đã có [`scripts/lint-migration-expand.ts`](../../../scripts/lint-migration-expand.ts) nên expand-contract là pattern có sẵn |
