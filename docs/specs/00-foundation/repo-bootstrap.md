...
| 10 | Chiến lược version control cho corpus spec ở workspace root. **Hai lượt quyết định:** | — | ✅ đóng | D-U → 👤 2026-08-07 |
...
| | **Lượt 2 — 2026-08-07 (quyết định người dùng)**: **đảo lại** — corpus spec ra khỏi `kidthink/`, về `docs/` ở workspace root (sibling của `kidthink/`), **không** track chung git repo code. Lý do: docs đổi nhịp khác code và người duyệt khác nhau (contract vs implementation) — tách để diff/review code không lẫn thay đổi markdown. `kidthink/SPEC.md` vẫn là symlink, target đổi `docs/SPEC.md` → `../docs/SPEC.md`. `scripts/lint-specs-lib.ts` thêm `CORPUS_ROOT = resolve(ROOT, "..", "docs")` — mọi path spec đọc qua biến này, không hardcode `kidthink/docs`. | | | |
