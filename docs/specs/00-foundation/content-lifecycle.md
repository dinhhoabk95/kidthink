...
| Field | Ghi chú |
|---|---|
| `entity_type` `entity_id` `content_version` | Polymorphic — toàn vẹn do service giữ |
| `from_status` `to_status` | |
| `actor_manager_id` `actor_role` | |
| `reason` | Bắt buộc khi `to_status = 'rejected'` |
| `checklist_snapshot` | JSONB — kết quả §7.3 tại thời điểm chuyển |
| `created_at` | |
