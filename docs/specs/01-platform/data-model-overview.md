...
| `BR-DM-13` | FK/quan hệ đa hình **nội bộ** mặc định lưu `id` (bigserial PK) của bảng đích. Ngoại lệ: (a) taxonomy Lớp 1 + `game_templates` (`competencies`·`strands`·`skills`·`learning_objectives`·`content_tags`·`game_templates` — PK thật là `code`); (b) tham chiếu cố ý luôn theo bản `published` mới nhất, ❌ không ghim version (`curriculum_items.entity_code`, `current_curriculum_code`, `activities.ref_code`) | `BR-DM-10` nói về lớp đối ngoại; áp nhầm sang FK nội bộ từng gây `id`→`code` không cần thiết (D-AE, 2026-08-07) |
...
| D-AE | Làm rõ `BR-DM-10` (chỉ áp lớp đối ngoại) + thêm `BR-DM-13` — FK/quan hệ đa hình nội bộ mặc định dùng `id`, trừ taxonomy Lớp 1+`game_templates` (dùng `code`) và tham chiếu cố ý luôn theo bản published mới nhất (dùng `code`) | 2026-08-07 | `content_review_log.entity_id` (SIB) · `lesson_activities.lesson_id` · `curriculum_items.curriculum_id` · `curriculum_enrollments.curriculum_id` · `play_sessions`/`telemetry_events`/`level_daily_stats`/`level_params`.`game_level_id` · `play_sessions.curriculum_id`/`lesson_id` · `content_asset_refs.entity_id` |
