<template>
  <div class="p-8 max-w-6xl mx-auto space-y-6">
    ...
    >
      ...
      <button
        class="px-5 py-2.5 rounded-2xl bg-brand-600 hover:bg-brand-700 active:scale-95 text-white font-semibold text-sm shadow-sm transition-all flex items-center gap-2"
        type="button"
        ...
        >
          <div class="space-y-1">
            <div class="flex items-center gap-2.5">
              <span
                class="font-mono text-xs px-2.5 py-0.5 rounded-full bg-brand-50 dark:bg-brand-900/40 text-brand-700 dark:text-brand-300 font-bold"
              >
                /{{ p.slug }}
              </span>
              <span
              ...
              <span
                class="text-xs px-2.5 py-0.5 rounded-full bg-success-100 text-success-800 font-semibold"
              >
                {{ p.status }}
              </span>
            </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            <button
              class="px-4 py-2 rounded-xl border border-surface-300 dark:border-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700 text-surface-700 dark:text-surface-200 text-xs font-semibold"
              type="button"
    ...
    >
      <div
        class="w-full max-w-2xl bg-white dark:bg-surface-800 rounded-3xl border-4 border-surface-200 dark:border-surface-700 p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto"
      >
        ...
        <div>
          ...
          <input
            class="w-full min-h-11 px-3 py-2 text-sm rounded-xl border-2 border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-900 text-surface-900 dark:text-white focus:border-brand-500 focus:outline-none font-mono"
            id="seo-slug"
        ...
        <div>
          ...
          <select
            class="w-full min-h-11 px-3 py-2 text-sm rounded-xl border-2 border-surface-300 dark:border-surface-600 bg-white dark:bg-surface-900 text-surface-900 dark:text-white"
            id="seo-type"
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            <label
              class="block text-xs font-bold text-surface-700 dark:text-surface-300"
              for="seo-title"
            >
              Tiêu đề thẻ Title *
            </label>
            <span
              :class="['text-xs font-semibold', formData.title.length > 60 ? 'text-warning-500' : 'text-surface-400']"
            >
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            ...
            <span
              :class="['text-xs font-semibold', formData.meta_description.length > 160 ? 'text-warning-500' : 'text-surface-400']"
            >
        ...
        <div
          class="p-4 rounded-2xl bg-surface-50 dark:bg-surface-900/50 border border-surface-200 dark:border-surface-700 space-y-1 text-left"
        >
          <span class="text-xs font-bold text-surface-500 block mb-2">
          ...
          <div
            class="text-base text-brand-600 dark:text-brand-400 font-medium hover:underline cursor-pointer"
          >
            {{ formData.title || 'Tiêu đề trang sẽ hiển thị ở đây' }}
          </div>
          <div
            class="text-xs text-surface-600 dark:text-surface-300 line-clamp-2"
          >
