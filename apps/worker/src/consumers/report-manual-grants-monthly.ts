...
export const reportManualGrantsMonthly: Consumer<
  "report:manual-grants-monthly"
> = async (payload, ctx) => {
  const result = await runManualGrantReportJob(ctx.jobId, {
    month: payload.month,
  });

  logJobDone("report:manual-grants-monthly", ctx, {
    month: result.month,
    grants: result.grants_count,
