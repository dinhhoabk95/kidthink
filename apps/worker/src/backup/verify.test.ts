...
describe("apps/worker/backup/verify", () => {
  ...
  it("fails if no successful backup found", async () => {
    ...
    await expect(runVerifyBackup("test-job-verify")).rejects.toThrow(
      "No successful backup found to verify"
    );
  });
  ...
  it("Ca âm BR-BAK-04: psql restore fail -> status = failed (via DB update)", async () => {
    ...
    await expect(runVerifyBackup("test-job-verify-fail")).rejects.toThrow(
      "psql restore failed"
    );
  ...
  it("restored_rows = 0 là fail, gọi alert mức cao", async () => {
    ...
    await expect(runVerifyBackup("test-job-verify-fail-0")).rejects.toThrow(
      "Verify failed: restored 0 rows"
    );
  ...
  it("Restore bản mã hoá mới nhất → verify schema → select count → xoá → ghi log", async () => {
    ...
    await runVerifyBackup("test-job-verify-success");
