...
import { activeSessions, getOwnerDb, users } from "@kidthink/db";
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // BR-ACS-02: Increment refresh_token_version + session_version and revoke all other sessions
    await db
      .update(users)
      .set({
        passwordHash: newHash,
        refreshTokenVersion: account.refreshTokenVersion + 1,
