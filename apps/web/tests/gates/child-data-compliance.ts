...
/** Route thật của apps/web mà cổng tuân thủ dữ liệu trẻ em phải phủ. */
export function readComplianceRouteFiles(): CodeFileFixture[] {
  return [
    ...scanDirectoryFiles(join(REPO_ROOT, "apps/web/server/api/managers")),
    ...scanDirectoryFiles(join(REPO_ROOT, "apps/web/server/api/users")),
    ...scanDirectoryFiles(join(REPO_ROOT, "apps/web/server/api/guest")),
  ];
}
