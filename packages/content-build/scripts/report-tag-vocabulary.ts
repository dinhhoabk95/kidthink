...
// Script sống trong packages/db, nhưng in đường dẫn theo gốc repo cho dễ dò.
const PACKAGE_ROOT = resolve(import.meta.dirname, "..");
const SEED_CONTENT_DIR = join(PACKAGE_ROOT, "src/seed-content");
const CONTENT_TAGS_FILE = join(PACKAGE_ROOT, "src/seed-master/content-tags.ts");
  ...
  for (const file of files) {
    ...
    while (match !== null) {
      for (const raw of (match[1] as string).split(",")) {
        ...
        seen.add(relative(PACKAGE_ROOT, file));
        byFile.set(tag, seen);
      }
