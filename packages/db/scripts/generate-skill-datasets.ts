...
} from "../src/seed-content/gates/skill-template-affinity.js";
import {
  type ParsedSkill,
  parseTaxonomyDocs,
...
const SKILLS_DIR = path.resolve(__dirname, "../src/seed-content/skills");
...
export function generateAllSkillDatasets(): {
  ...
  for (const skill of skills) {
    ...
    const levelPlans: SkillLevelPlan[] = targetTemplates.map((t, idx) => ({
      template: t,
      band,
