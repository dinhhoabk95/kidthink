...
describe("Sàn chiều sâu mỗi engine — Task #122 (BR-ECD-01..13)", () => {
  it("Bậc 0: toàn bộ 32 engine active đều đạt sàn baseline", () => {
    ...
    expect(report.totalEngines).toBe(32);
    expect(report.passedEnginesCount).toBe(32);
  ...
  it("Scenario: BR-ECD-10 — báo cáo không chứa phần trăm tổng (%)", () => {
    ...
    expect(formatted).toContain("32 engine active");
  });
