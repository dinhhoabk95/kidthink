...
import { TAG_VOCABULARY } from "#src/seed-content/vocabulary";
import { normalizeMechanicTagCode } from "#src/services/tagging";
...
const VOCABULARY_TAIL_TAGS: TagSeedItem[] = (
  ...
  TAG_VOCABULARY[axis]
    .filter((code) => !HAND_LABELLED_CODES.has(code))
    .filter((code) => (AXIS_TIEBREAK[code] ?? axis) === axis)
    .map((code) => ({ code, axis, label: code }))
);
