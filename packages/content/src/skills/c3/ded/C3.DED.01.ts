import type { SkillDataset, SkillIdentity, SkillSeed } from "@mindkid/shared";

export const C3_DED_01_IDENTITY: SkillIdentity = {
  code: "C3.DED.01",
  strand_code: "C3.DED",
  competency_code: "C3",
  name: "Loại trừ để tìm đáp án",
  age_min: 5,
  age_max: 5,
  difficulty: 4,
  thinking_processes: ["deduce"],
  tier: "advanced",
  prerequisites: ["C3.CLS.06"],
  learning_objectives: [
    {
      code: "LO-C3.DED.01-01",
      behaviour: "Nhận biết và thực hành Loại trừ để tìm đáp án ở mức cơ bản",
      observable_criteria:
        "Trẻ thực hiện đúng không qua gợi ý trong 3 lần thử liên tiếp.",
      position: 1,
    },
    {
      code: "LO-C3.DED.01-02",
      behaviour: "Vận dụng Loại trừ để tìm đáp án trong môi trường tương tác",
      observable_criteria:
        "Trẻ hoàn thành thử thách độc lập trong vòng 2 phút.",
      position: 2,
    },
    {
      code: "LO-C3.DED.01-03",
      behaviour:
        "Giải quyết vấn đề nâng cao liên quan tới Loại trừ để tìm đáp án",
      observable_criteria:
        "Trẻ trả lời chính xác câu hỏi phân loại/suy luận liên quan.",
      position: 3,
    },
  ],
};

export const C3_DED_01_DATASET: SkillDataset = {
  skill_code: "C3.DED.01",
  concept_label: "Loại trừ để tìm đáp án",
  surface: "worksheet",
  items: [
    {
      id: "banana",
      label: "quả chuối",
      image: {
        kind: "emoji",
        ref: "🍌",
      },
      category: {
        type: "hoa quả",
      },
    },
    {
      id: "watermelon",
      label: "dưa hấu",
      image: {
        kind: "emoji",
        ref: "🍉",
      },
      category: {
        type: "hoa quả",
      },
    },
    {
      id: "carrot",
      label: "củ cà rốt",
      image: {
        kind: "emoji",
        ref: "🥕",
      },
      category: {
        type: "rau củ",
      },
    },
    {
      id: "corn",
      label: "bắp ngô",
      image: {
        kind: "emoji",
        ref: "🌽",
      },
      category: {
        type: "rau củ",
      },
    },
    {
      id: "dog",
      label: "con chó",
      image: {
        kind: "emoji",
        ref: "🐕",
      },
      category: {
        type: "động vật",
      },
    },
  ],
  ladder: [
    {
      rung: 1,
      dimension: "range",
      description: "Làm quen cơ bản với Loại trừ để tìm đáp án",
    },
    {
      rung: 2,
      dimension: "range",
      description: "Nhận biết và chọn đúng Loại trừ để tìm đáp án",
    },
    {
      rung: 3,
      dimension: "distractor_count",
      description: "Phân biệt với phương án nhiễu",
    },
    {
      rung: 4,
      dimension: "item_count",
      description: "Mở rộng phạm vi và số lượng",
    },
    {
      rung: 5,
      dimension: "speed_scaffolding",
      description: "Thuần thục và độc lập thực hiện",
    },
  ],
  phrasing: {
    prompt_template: "Bé hãy chọn đúng {label} nhé!",
    narration_template:
      "Chúng mình cùng tìm hiểu về Loại trừ để tìm đáp án nhé",
    success_message: "Hoan hô, bé đã chọn đúng rồi!",
    hint_message: "Bé hãy nhìn kỹ hình {label} nhé!",
  },
  ordering: ["banana", "watermelon", "carrot", "corn", "dog"],
};

export const C3_DED_01_SEED: SkillSeed = {
  identity: C3_DED_01_IDENTITY,
  dataset: C3_DED_01_DATASET,
  levels: [
    {
      code: "GL-C3-LOG-CMP-0008",
      template: "GT-001",
      band: "4-5",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
    {
      code: "GL-C3-LOG-CMP-0009",
      template: "GT-001",
      band: "4-5",
      difficulty: 2,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C3-LOG-POS-0011",
      template: "GT-022",
      band: "4-5",
      difficulty: 2,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C3-LOG-CMP-0015",
      template: "GT-001",
      band: "5-6",
      difficulty: 3,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C3-LOG-POS-0017",
      template: "GT-022",
      band: "5-6",
      difficulty: 4,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C3-LOG-CMP-0020",
      template: "GT-001",
      band: "5-6",
      difficulty: 4,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C3-CLU-DED-0021",
      template: "GT-009",
      band: "4-5",
      difficulty: 2,
      theme: "school",
      rounds: 3,
      legacy_v1_ref: "D6-07",
    },
    {
      code: "GL-C3-CLU-DED-0022",
      template: "GT-009",
      band: "5-6",
      difficulty: 3,
      theme: "school",
      rounds: 3,
      legacy_v1_ref: "D6-07",
    },
    {
      code: "GL-C3-CLU-DED-0023",
      template: "GT-009",
      band: "5-6",
      difficulty: 4,
      theme: "school",
      rounds: 3,
      legacy_v1_ref: "D6-07",
    },
    {
      code: "GL-C3-SUD-MIN-0027",
      template: "GT-015",
      band: "5-6",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
      legacy_v1_ref: "D6-02",
    },
    {
      code: "GL-C3-SUD-MIN-0028",
      template: "GT-015",
      band: "5-6",
      difficulty: 3,
      theme: "food",
      rounds: 3,
      legacy_v1_ref: "D6-02",
    },
    {
      code: "GL-C3-SUD-MIN-0029",
      template: "GT-015",
      band: "5-6",
      difficulty: 4,
      theme: "school",
      rounds: 3,
      legacy_v1_ref: "D6-02",
    },
    {
      code: "GL-C3-RLF-BSK-0003",
      template: "GT-003",
      band: "3-4",
      difficulty: 3,
      theme: "food",
      rounds: 3,
      legacy_v1_ref: "D4-08",
    },
    {
      code: "GL-C3-RLF-BSK-0004",
      template: "GT-003",
      band: "4-5",
      difficulty: 1,
      theme: "animal",
      rounds: 3,
      legacy_v1_ref: "D4-08",
    },
    {
      code: "GL-C3-RLF-BSK-0005",
      template: "GT-003",
      band: "4-5",
      difficulty: 2,
      theme: "nature",
      rounds: 3,
      legacy_v1_ref: "D4-08",
    },
    {
      code: "GL-C3-DED-TAP-0001",
      template: "GT-001",
      band: "4-5",
      difficulty: 3,
      theme: "art",
      rounds: 3,
    },
    {
      code: "GL-C3-DED-TCNT-0001",
      template: "GT-002",
      band: "4-5",
      difficulty: 3,
      theme: "festival",
      rounds: 3,
    },
    {
      code: "GL-C3-DED-TCNT-0002",
      template: "GT-002",
      band: "4-5",
      difficulty: 4,
      theme: "job",
      rounds: 3,
    },
    {
      code: "GL-C3-DED-TCNT-0003",
      template: "GT-002",
      band: "4-5",
      difficulty: 5,
      theme: "homeland",
      rounds: 3,
    },
    {
      code: "GL-C3-DED-TCNT-0004",
      template: "GT-002",
      band: "4-5",
      difficulty: 3,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C3-DED-TCNT-0005",
      template: "GT-002",
      band: "4-5",
      difficulty: 4,
      theme: "farm",
      rounds: 3,
    },
  ],
};
