import { getPatternUnit } from "#src/inventories/index";

export const C1_PAT_10_UNIT = getPatternUnit("pat_abc");

import type { SkillDataset, SkillIdentity, SkillSeed } from "@mindkid/shared";

export const C1_PAT_10_IDENTITY: SkillIdentity = {
  code: "C1.PAT.10",
  strand_code: "C1.PAT",
  competency_code: "C1",
  name: "Pattern màu",
  age_min: 3,
  age_max: 3,
  difficulty: 1,
  thinking_processes: ["observe", "predict"],
  tier: "basic",
  prerequisites: ["C1.PAT.01"],
  learning_objectives: [
    {
      code: "LO-C1.PAT.10-01",
      behaviour: "Nhận biết và thực hành Pattern màu ở mức cơ bản",
      observable_criteria:
        "Trẻ thực hiện đúng không qua gợi ý trong 3 lần thử liên tiếp.",
      position: 1,
    },
    {
      code: "LO-C1.PAT.10-02",
      behaviour: "Vận dụng Pattern màu trong môi trường tương tác",
      observable_criteria:
        "Trẻ hoàn thành thử thách độc lập trong vòng 2 phút.",
      position: 2,
    },
    {
      code: "LO-C1.PAT.10-03",
      behaviour: "Giải quyết vấn đề nâng cao liên quan tới Pattern màu",
      observable_criteria:
        "Trẻ trả lời chính xác câu hỏi phân loại/suy luận liên quan.",
      position: 3,
    },
  ],
};

export const C1_PAT_10_DATASET: SkillDataset = {
  skill_code: "C1.PAT.10",
  concept_label: "Pattern màu",
  surface: "game",
  items: [
    {
      id: "pat_blue",
      label: "màu xanh dương",
      value: 2,
      image: {
        kind: "emoji",
        ref: "🔵",
      },
      contrast_group: "color",
    },
    {
      id: "pat_yellow",
      label: "màu vàng",
      value: 3,
      image: {
        kind: "emoji",
        ref: "🟡",
      },
      contrast_group: "color",
    },
    {
      id: "pat_red",
      label: "màu đỏ",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🔴",
      },
      contrast_group: "color",
    },
  ],
  relations: [
    {
      type: "sequence",
      source_id: "pat_red",
      target_id: "pat_blue",
    },
    {
      type: "sequence",
      source_id: "pat_blue",
      target_id: "pat_yellow",
    },
    {
      type: "sequence",
      source_id: "pat_yellow",
      target_id: "pat_red",
    },
  ],
  ladder: [
    {
      rung: 1,
      dimension: "range",
      description: "Làm quen cơ bản với pattern màu",
    },
    {
      rung: 2,
      dimension: "range",
      description: "Nhận biết và chọn đúng pattern màu",
    },
    {
      rung: 3,
      dimension: "distractor_count",
      description: "Phân biệt pattern màu với phương án nhiễu",
    },
    {
      rung: 4,
      dimension: "item_count",
      description: "Mở rộng phạm vi pattern màu",
    },
    {
      rung: 5,
      dimension: "speed_scaffolding",
      description: "Thuần thục pattern màu và tự làm một mình",
    },
  ],
  phrasing: {
    prompt_template: "Màu tiếp theo theo đúng quy luật là màu gì hả bé?",
    narration_template: "Chúng mình cùng tìm hiểu về Pattern màu nhé",
  },
  ordering: ["pat_red", "pat_blue", "pat_yellow"],
};

export const C1_PAT_10_SEED: SkillSeed = {
  identity: C1_PAT_10_IDENTITY,
  dataset: C1_PAT_10_DATASET,
  levels: [
    {
      code: "GL-C1-PAT-TAP-0025",
      template: "GT-001",
      band: "3-4",
      difficulty: 1,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TAP-0026",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "farm",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TAP-0027",
      template: "GT-001",
      band: "3-4",
      difficulty: 1,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TAP-0028",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TCMP-0025",
      template: "GT-003",
      band: "3-4",
      difficulty: 1,
      theme: "nature",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TCMP-0026",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "ocean",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TCMP-0027",
      template: "GT-003",
      band: "3-4",
      difficulty: 1,
      theme: "food",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-TCMP-0028",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "vehicle",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-PATT-0025",
      template: "GT-005",
      band: "3-4",
      difficulty: 1,
      theme: "art",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-PATT-0026",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "art",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-PATT-0027",
      template: "GT-005",
      band: "3-4",
      difficulty: 1,
      theme: "family",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-PATT-0028",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "body",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-SLOT-0008",
      template: "GT-008",
      band: "3-4",
      difficulty: 1,
      theme: "weather",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-SLOT-0009",
      template: "GT-008",
      band: "3-4",
      difficulty: 2,
      theme: "festival",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-SLOT-0010",
      template: "GT-008",
      band: "3-4",
      difficulty: 1,
      theme: "job",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-SLOT-0011",
      template: "GT-008",
      band: "3-4",
      difficulty: 2,
      theme: "homeland",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-MEMO-0009",
      template: "GT-012",
      band: "3-4",
      difficulty: 1,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-MEMO-0010",
      template: "GT-012",
      band: "3-4",
      difficulty: 2,
      theme: "farm",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-MEMO-0011",
      template: "GT-012",
      band: "3-4",
      difficulty: 1,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-PAT-MEMO-0012",
      template: "GT-012",
      band: "3-4",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
  ],
};
