import { getQuantityRep } from "#src/inventories/index";

export const C1_OTO_05_REP = getQuantityRep("rep_discrete_object");

import type { SkillDataset, SkillIdentity, SkillSeed } from "@mindkid/shared";

export const C1_OTO_05_IDENTITY: SkillIdentity = {
  code: "C1.OTO.05",
  strand_code: "C1.OTO",
  competency_code: "C1",
  name: "Ghép theo vị trí",
  age_min: 4,
  age_max: 4,
  difficulty: 3,
  thinking_processes: ["match", "observe"],
  tier: "core",
  prerequisites: ["C1.OTO.01"],
  learning_objectives: [
    {
      code: "LO-C1.OTO.05-01",
      behaviour: "Nhận biết và thực hành Ghép theo vị trí ở mức cơ bản",
      observable_criteria:
        "Trẻ thực hiện đúng không qua gợi ý trong 3 lần thử liên tiếp.",
      position: 1,
    },
    {
      code: "LO-C1.OTO.05-02",
      behaviour: "Vận dụng Ghép theo vị trí trong môi trường tương tác",
      observable_criteria:
        "Trẻ hoàn thành thử thách độc lập trong vòng 2 phút.",
      position: 2,
    },
    {
      code: "LO-C1.OTO.05-03",
      behaviour: "Giải quyết vấn đề nâng cao liên quan tới Ghép theo vị trí",
      observable_criteria:
        "Trẻ trả lời chính xác câu hỏi phân loại/suy luận liên quan.",
      position: 3,
    },
  ],
};

export const C1_OTO_05_DATASET: SkillDataset = {
  skill_code: "C1.OTO.05",
  concept_label: "Ghép theo vị trí",
  surface: "game",
  items: [
    {
      id: "pos_top_1",
      label: "ở trên hàng một",
      value: 1,
      image: {
        kind: "emoji",
        ref: "⬆️",
      },
      contrast_group: "top",
    },
    {
      id: "pos_bottom_1",
      label: "ở dưới hàng một",
      value: 1,
      image: {
        kind: "emoji",
        ref: "⬇️",
      },
      contrast_group: "bottom",
    },
    {
      id: "pos_top_2",
      label: "ở trên hàng hai",
      value: 2,
      image: {
        kind: "emoji",
        ref: "⬆️",
      },
      contrast_group: "top",
    },
    {
      id: "pos_bottom_2",
      label: "ở dưới hàng hai",
      value: 2,
      image: {
        kind: "emoji",
        ref: "⬇️",
      },
      contrast_group: "bottom",
    },
    {
      id: "pos_top_3",
      label: "ở trên hàng ba",
      value: 3,
      image: {
        kind: "emoji",
        ref: "⬆️",
      },
      contrast_group: "top",
    },
    {
      id: "pos_bottom_3",
      label: "ở dưới hàng ba",
      value: 3,
      image: {
        kind: "emoji",
        ref: "⬇️",
      },
      contrast_group: "bottom",
    },
  ],
  relations: [
    {
      type: "pair",
      source_id: "pos_top_1",
      target_id: "pos_bottom_1",
    },
    {
      type: "pair",
      source_id: "pos_top_2",
      target_id: "pos_bottom_2",
    },
    {
      type: "pair",
      source_id: "pos_top_3",
      target_id: "pos_bottom_3",
    },
  ],
  ladder: [
    {
      rung: 1,
      dimension: "range",
      description: "Làm quen cơ bản với ghép theo vị trí",
    },
    {
      rung: 2,
      dimension: "range",
      description: "Nhận biết và chọn đúng ghép theo vị trí",
    },
    {
      rung: 3,
      dimension: "distractor_count",
      description: "Phân biệt ghép theo vị trí với phương án nhiễu",
    },
    {
      rung: 4,
      dimension: "item_count",
      description: "Mở rộng phạm vi ghép theo vị trí",
    },
    {
      rung: 5,
      dimension: "speed_scaffolding",
      description: "Thuần thục ghép theo vị trí và tự làm một mình",
    },
  ],
  phrasing: {
    prompt_template: "Bé hãy ghép đôi từng {label} tương ứng nhé!",
    narration_template: "Chúng mình cùng tìm hiểu về Ghép theo vị trí nhé",
  },
};

export const C1_OTO_05_SEED: SkillSeed = {
  identity: C1_OTO_05_IDENTITY,
  dataset: C1_OTO_05_DATASET,
  levels: [
    {
      code: "GL-C1-OTO-TAP-0017",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "weather",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0018",
      template: "GT-001",
      band: "3-4",
      difficulty: 3,
      theme: "festival",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0019",
      template: "GT-001",
      band: "3-4",
      difficulty: 4,
      theme: "job",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0020",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "homeland",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCNT-0001",
      template: "GT-002",
      band: "4-5",
      difficulty: 2,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCNT-0002",
      template: "GT-002",
      band: "4-5",
      difficulty: 3,
      theme: "farm",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCNT-0003",
      template: "GT-002",
      band: "4-5",
      difficulty: 4,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCNT-0004",
      template: "GT-002",
      band: "4-5",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0017",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "nature",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0018",
      template: "GT-003",
      band: "3-4",
      difficulty: 3,
      theme: "ocean",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0019",
      template: "GT-003",
      band: "3-4",
      difficulty: 4,
      theme: "food",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0020",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "vehicle",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0005",
      template: "GT-004",
      band: "4-5",
      difficulty: 2,
      theme: "art",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0006",
      template: "GT-004",
      band: "4-5",
      difficulty: 3,
      theme: "space",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0007",
      template: "GT-004",
      band: "4-5",
      difficulty: 4,
      theme: "family",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0008",
      template: "GT-004",
      band: "4-5",
      difficulty: 2,
      theme: "body",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0017",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "weather",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0018",
      template: "GT-005",
      band: "3-4",
      difficulty: 3,
      theme: "festival",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0019",
      template: "GT-005",
      band: "3-4",
      difficulty: 4,
      theme: "job",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0020",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "homeland",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-ROT-0001",
      template: "GT-019",
      band: "4-5",
      difficulty: 1,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-ROT-0002",
      template: "GT-019",
      band: "4-5",
      difficulty: 2,
      theme: "home",
      rounds: 3,
    },
  ],
};
