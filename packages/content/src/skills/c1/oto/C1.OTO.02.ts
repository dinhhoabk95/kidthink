import { getQuantityRep } from "#src/inventories/index";

export const C1_OTO_02_REP = getQuantityRep("rep_discrete_object");

import type { SkillDataset, SkillIdentity, SkillSeed } from "@mindkid/shared";

export const C1_OTO_02_IDENTITY: SkillIdentity = {
  code: "C1.OTO.02",
  strand_code: "C1.OTO",
  competency_code: "C1",
  name: "Ghép nhiều với một",
  age_min: 4,
  age_max: 4,
  difficulty: 3,
  thinking_processes: ["match", "infer"],
  tier: "core",
  prerequisites: ["C1.OTO.01"],
  learning_objectives: [
    {
      code: "LO-C1.OTO.02-01",
      behaviour: "Nhận biết và thực hành Ghép nhiều với một ở mức cơ bản",
      observable_criteria:
        "Trẻ thực hiện đúng không qua gợi ý trong 3 lần thử liên tiếp.",
      position: 1,
    },
    {
      code: "LO-C1.OTO.02-02",
      behaviour: "Vận dụng Ghép nhiều với một trong môi trường tương tác",
      observable_criteria:
        "Trẻ hoàn thành thử thách độc lập trong vòng 2 phút.",
      position: 2,
    },
    {
      code: "LO-C1.OTO.02-03",
      behaviour: "Giải quyết vấn đề nâng cao liên quan tới Ghép nhiều với một",
      observable_criteria:
        "Trẻ trả lời chính xác câu hỏi phân loại/suy luận liên quan.",
      position: 3,
    },
  ],
};

export const C1_OTO_02_DATASET: SkillDataset = {
  skill_code: "C1.OTO.02",
  concept_label: "Ghép nhiều với một",
  surface: "game",
  items: [
    {
      id: "hen",
      label: "gà mẹ",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🐔",
      },
      contrast_group: "mother",
      category: {
        species: "gà",
        role: "mẹ",
      },
    },
    {
      id: "chick_1",
      label: "gà con một",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🐥",
      },
      contrast_group: "baby",
      category: {
        species: "gà",
        role: "con",
      },
    },
    {
      id: "chick_2",
      label: "gà con hai",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🐥",
      },
      contrast_group: "baby",
      category: {
        species: "gà",
        role: "con",
      },
    },
    {
      id: "chick_3",
      label: "gà con ba",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🐥",
      },
      contrast_group: "baby",
      category: {
        species: "gà",
        role: "con",
      },
    },
    {
      id: "duck_mother",
      label: "vịt mẹ",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🦆",
      },
      contrast_group: "mother",
      category: {
        species: "vịt",
        role: "mẹ",
      },
    },
    {
      id: "duckling",
      label: "vịt con",
      value: 1,
      image: {
        kind: "emoji",
        ref: "🐤",
      },
      contrast_group: "baby",
      category: {
        species: "vịt",
        role: "con",
      },
    },
  ],
  relations: [
    {
      type: "pair",
      source_id: "chick_1",
      target_id: "hen",
    },
    {
      type: "pair",
      source_id: "chick_2",
      target_id: "hen",
    },
    {
      type: "pair",
      source_id: "chick_3",
      target_id: "hen",
    },
    {
      type: "pair",
      source_id: "duckling",
      target_id: "duck_mother",
    },
  ],
  axes: {
    species: {
      values: ["gà", "vịt"],
    },
    role: {
      values: ["mẹ", "con"],
    },
  },
  ladder: [
    {
      rung: 1,
      dimension: "range",
      description: "Làm quen cơ bản với ghép nhiều với một",
    },
    {
      rung: 2,
      dimension: "range",
      description: "Nhận biết và chọn đúng ghép nhiều với một",
    },
    {
      rung: 3,
      dimension: "distractor_count",
      description: "Phân biệt ghép nhiều với một với phương án nhiễu",
    },
    {
      rung: 4,
      dimension: "item_count",
      description: "Mở rộng phạm vi ghép nhiều với một",
    },
    {
      rung: 5,
      dimension: "speed_scaffolding",
      description: "Thuần thục ghép nhiều với một và tự làm một mình",
    },
  ],
  phrasing: {
    prompt_template: "Bé hãy ghép đôi từng {label} tương ứng nhé!",
    narration_template: "Chúng mình cùng tìm hiểu về Ghép nhiều với một nhé",
  },
};

export const C1_OTO_02_SEED: SkillSeed = {
  identity: C1_OTO_02_IDENTITY,
  dataset: C1_OTO_02_DATASET,
  levels: [
    {
      code: "GL-C1-OTO-TAP-0005",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0006",
      template: "GT-001",
      band: "3-4",
      difficulty: 3,
      theme: "farm",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0007",
      template: "GT-001",
      band: "3-4",
      difficulty: 4,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TAP-0008",
      template: "GT-001",
      band: "3-4",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0005",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "nature",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0006",
      template: "GT-003",
      band: "3-4",
      difficulty: 3,
      theme: "ocean",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0007",
      template: "GT-003",
      band: "3-4",
      difficulty: 4,
      theme: "food",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-TCMP-0008",
      template: "GT-003",
      band: "3-4",
      difficulty: 2,
      theme: "vehicle",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0001",
      template: "GT-004",
      band: "4-5",
      difficulty: 2,
      theme: "art",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0002",
      template: "GT-004",
      band: "4-5",
      difficulty: 3,
      theme: "space",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0003",
      template: "GT-004",
      band: "4-5",
      difficulty: 4,
      theme: "family",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PAIR-0004",
      template: "GT-004",
      band: "4-5",
      difficulty: 2,
      theme: "body",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0005",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "weather",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0006",
      template: "GT-005",
      band: "3-4",
      difficulty: 3,
      theme: "festival",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0007",
      template: "GT-005",
      band: "3-4",
      difficulty: 4,
      theme: "job",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-PATT-0008",
      template: "GT-005",
      band: "3-4",
      difficulty: 2,
      theme: "homeland",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-SHAD-0001",
      template: "GT-007",
      band: "3-4",
      difficulty: 2,
      theme: "school",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-SHAD-0002",
      template: "GT-007",
      band: "3-4",
      difficulty: 3,
      theme: "farm",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-SHAD-0003",
      template: "GT-007",
      band: "3-4",
      difficulty: 4,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-SHAD-0004",
      template: "GT-007",
      band: "3-4",
      difficulty: 2,
      theme: "animal",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-MIR-0001",
      template: "GT-021",
      band: "4-5",
      difficulty: 1,
      theme: "home",
      rounds: 3,
    },
    {
      code: "GL-C1-OTO-MIR-0002",
      template: "GT-021",
      band: "4-5",
      difficulty: 2,
      theme: "food",
      rounds: 3,
    },
  ],
};
