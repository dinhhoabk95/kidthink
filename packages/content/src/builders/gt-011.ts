...
export const projectGT011: Projection<"GT-011"> = {
  ...
  project(dataset: SkillDataset, opts: ProjectOptions): ProjectedPack {
    ...
    const cells = [
      ...
      { row: 1, col: 0, asset: resolveItemAsset(itemA, true) },
      { row: 1, col: 1, asset: null },
    ];

    const distractorItem = shuffled[2] ?? itemA;
    const options = [
      {
      ...
      {
        option_id: "opt_d1",
        asset: resolveItemAsset(distractorItem, true),
        is_correct: false,
      },
      {
        option_id: "opt_d2",
    ...
    return {
      content_pack: {
        prompt:
          "Bé hãy tìm hình thích hợp để điền vào ô trống theo quy luật nhé!",
        matrix: {
          rows: 2 as const,
          cols: 2 as const,
          cells,
