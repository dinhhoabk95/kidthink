...
/**
 * Cổng `lint:specs` cũ là CLI: nó chạy **cả 18 check** trên corpus thật rồi
 * exit 1. Test theo từng check ở trên không thay được điều đó — mỗi cái chỉ gọi
 * đúng check của nó. Đây là phần thay cho vỏ CLI.
 */
describe("Cổng lint:specs trên corpus thật", () => {
  it("18 check đều xanh trên docs/specs", () => {
    const specs = collectSpecFiles();
  ...
  it("thật sự đọc được corpus — không xanh vì danh sách spec rỗng", () => {
    expect(collectSpecFiles().length).toBeGreaterThan(140);
