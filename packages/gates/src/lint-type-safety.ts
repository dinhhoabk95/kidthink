 ...
 * với `packages/gates/src/type-safety-baseline.json`:
 *   - file tăng số  → fail
 ...
 *
 * Chạy `node packages/gates/src/lint-type-safety.ts` hoặc `--update` để hạ baseline.
 */
...
const ROOTS = ["apps", "packages", "scripts"].map((dir) => repoPath(dir));
...
export function scanRepo(roots: string[] = ROOTS): Baseline {
  ...
  for (const root of roots) {
    for (const file of walkSource(root)) {
      // Khoá baseline luôn là đường dẫn **tương đối gốc repo**: root quét là
      // đường dẫn tuyệt đối (cổng không phụ thuộc cwd), nhưng baseline phải đọc
      // được và so được giữa các máy.
