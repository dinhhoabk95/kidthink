...
} from "../packages/shared/src/config-dictionary.ts";
import {
  getMaxNestingDepth,
  introspectZodSchema,
  type UiHintResult,
} from "../packages/shared/src/zod-introspect.ts";
...
if (process.argv[1]?.endsWith("lint-form-gates.ts")) {
  const violations = runAllFormGates();
  if (violations.length === 0) {
    process.stdout.write(
      "✅ [lint:form-gates] All 4 schema-driven form gates passed cleanly.\n"
    );
    process.exit(0);
  }

  process.stderr.write(
    `❌ [lint:form-gates] Found ${violations.length} anti-drift violations:\n`
  );
