 ...
 */
export function scanAllMigrations(root = process.cwd()): MigrationScanResult {
  const files: string[] = [];
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (scannedFiles === 0) {
    console.error(
      `❌ [lint:migration-expand] Scanned 0 migration files. Expected them under one of: ${MIGRATION_DIRS.join(", ")}`
    );
    process.exit(1);
  }
  ...
  if (violations.length === 0) {
    console.log(
      `✅ [lint:migration-expand] ${scannedFiles} migration files are expand-only (BR-RBK-02).`
    );
    process.exit(0);
  }

  console.error(
    `❌ [lint:migration-expand] ${violations.length} destructive statements in ${scannedFiles} files:`
  );
  for (const v of violations) {
    console.error(`  ${v.file}:${v.line} — ${v.pattern}: ${v.codeSnippet}`);
  }
  process.exit(1);
}
