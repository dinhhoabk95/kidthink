...
export function main(): void {
  ...
  if (violations.length === 0) {
    ...
    process.stdout.write(
      `✅ lint:rule-ids — ${rules.length} business rules, bất biến BR-REG2-01..04 toàn vẹn.\n`
    );
    process.exit(0);
  }

  process.stderr.write(
    `❌ lint:rule-ids — phát hiện ${violations.length} vi phạm quản trị rule:\n\n`
  );
  for (const v of violations) {
    const loc = v.line ? `${v.file}:${v.line}` : v.file;
    process.stderr.write(`  [${v.rule}] ${loc} — ${v.message}\n`);
  }
  process.stderr.write("\n");
  process.exit(1);
}
