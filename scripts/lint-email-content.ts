...
// CLI runner
if (process.argv[1]?.endsWith("lint-email-content.ts")) {
  const templatesDir = path.join(process.cwd(), "apps/web/server/templates");
  const violations = lintEmailTemplates(templatesDir);

  if (violations.length > 0) {
    console.error("❌ [lint:email] Content violations found:");
  ...
  console.log(
    "✅ [lint:email] BR-NOT-03, BR-NOT-07, BR-NOT-08 passed cleanly."
  );
}
